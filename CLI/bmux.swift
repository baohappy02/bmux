...
private final class CLISocketSentryTelemetry {
    ...
    func captureError(stage: String, error: Error) {
        ...
        _ = SentrySDK.capture(error: error) { scope in
            ...
            scope.setTag(value: "bmux-cli", key: "component")
    ...
    private var shouldEmit: Bool {
        !disabledByEnv && !Self.dsn.isEmpty
    }
...
private struct ClaudeHookSessionRecord: Codable {
    ...
    var observedCompactionCount: Int?
...
private struct CodexHookCompactionSnapshot {
    let transcriptPath: String
    let compactionCount: Int
}

private final class ClaudeHookSessionStore {
    private static let defaultStatePath = "~/.bmuxterm/claude-hook-sessions.json"
    ...
    func upsert(
        ...
        observedCompactionCount: Int? = nil
    ) throws {
        let normalized = normalizeSessionId(sessionId)
        ...
        try withLockedState { state in
            ...
            if let observedCompactionCount {
                record.observedCompactionCount = observedCompactionCount
            }
...
enum SocketPasswordResolver {
    private static let service = "com.bmuxterm.app.socket-control"
    private static let account = "local-socket-password"
    private static let directoryName = "bmux"
...
private enum CLISocketPathResolver {
    private static let appSupportDirectoryName = "bmux"
    ...
    private static let legacyLastSocketPathFile = "/tmp/bmux-last-socket-path"
    ...
    private static func candidatePaths(requestedPath: String, environment: [String: String]) -> [String] {
        ...
        if let tag = normalized(environment["CMUX_TAG"]) {
            ...
            candidates.append("/tmp/bmux-debug-\(slug).sock")
            candidates.append("/tmp/bmux-\(slug).sock")
        }
    ...
    private static func discoverTaggedSockets(limit: Int) -> [String] {
        ...
        for directory in socketDiscoveryDirectories() {
            ...
            for name in entries where name.hasPrefix("bmux") && name.hasSuffix(".sock") {
...
final class SocketClient {
    ...
    static func waitForConnectableSocket(path: String, timeout: TimeInterval) throws -> SocketClient {
        ...
        guard let watchDirectory = existingWatchDirectory(forPath: path) else {
            throw CLIError(message: "bmux app did not start in time (socket not found at \(path))")
        }
        ...
        let queue = DispatchQueue(label: "com.bmux.cli.socket-watch.\(UUID().uuidString)")
    ...
    static func waitForFilesystemPath(_ path: String, timeout: TimeInterval) throws {
        ...
        let queue = DispatchQueue(label: "com.bmux.cli.path-watch.\(UUID().uuidString)")
        let semaphore = DispatchSemaphore(value: 0)
        var found = false
        let source = DispatchSource.makeFileSystemObjectSource(
            fileDescriptor: watchFD,
...
struct CMUXCLI {
    ...
    private static let debugLastSocketHintPath = "/tmp/bmux-last-socket-path"
    ...
    private static func debugSocketPathFromHintFile() -> String? {
        ...
        guard let hinted = normalizedEnvValue(raw),
              hinted.hasPrefix("/tmp/bmux-debug"),
    ...
    func run() throws {
        ...
        if command != "__tmux-compat",
           ...
           (commandArgs.contains("--help") || commandArgs.contains("-h")) {
            ...
            print("Unknown command '\(command)'. Run 'bmux help' to see available commands.")
            return
        }
        ...
        // Codex hook handler: gracefully no-op when not inside bmux
    ...
    private func runMarkdownCommand(
        ...
        if let first = args.first, first.lowercased() == "open" {
            ...
            if let first = args.first, first.hasPrefix("-") {
                throw CLIError(
                    message:
                        "markdown open: unknown flag '\(first)'. Usage: bmux markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>] [--direction right|down|left|up]"
                )
            } else if let first = args.first, looksLikePath(first) || first.contains(".") {
                subArgs = args
            } else if let first = args.first {
                throw CLIError(message: "Unknown markdown subcommand: \(first). Usage: bmux markdown open <path>")
            } else {
                subArgs = []
            }
        }
        ...
        guard let rawPath = subArgs.first, !rawPath.isEmpty else {
            throw CLIError(message: "markdown open requires a file path. Usage: bmux markdown open <path>")
        }
        let trailingArgs = Array(subArgs.dropFirst())
        if let unknownFlag = trailingArgs.first(where: { $0.hasPrefix("-") }) {
            throw CLIError(
                message:
                    "markdown open: unknown flag '\(unknownFlag)'. Usage: bmux markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>] [--direction right|down|left|up]"
            )
        }
        if let extraArg = trailingArgs.first {
            throw CLIError(
                message:
                    "markdown open: unexpected argument '\(extraArg)'. Usage: bmux markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>] [--direction right|down|left|up]"
            )
        }
    ...
    /// Open a path in bmux by creating a new workspace with the given directory.
    ...
    private func runSSH(
        ...
        do {
            ...
            // `bmux ssh` is an explicit "open this remote workspace now" action,
    ...
    private func parseSSHCommandOptions(_ commandArgs: [String], localSocketPath: String = "", remoteRelayPort: Int = 0) throws -> SSHCommandOptions {
        ...
        guard let destination else {
            throw CLIError(message: "ssh requires a destination (example: bmux ssh user@host)")
        }
    ...
    private func buildSSHBootstrapCommandSnippet(
        ...
        var lines: [String] = [
            "bmux_workspace_id=\"${CMUX_WORKSPACE_ID:-}\"",
            "bmux_surface_id=\"${CMUX_SURFACE_ID:-}\"",
            "bmux_remote_bootstrap_b64=\(shellQuote(encodedBootstrapScript))",
            "bmux_remote_bootstrap=\"$(printf %s \"$bmux_remote_bootstrap_b64\" | base64 -d 2>/dev/null || printf %s \"$bmux_remote_bootstrap_b64\" | base64 -D 2>/dev/null)\"",
            "bmux_remote_bootstrap=\"$(printf '%s' \"$bmux_remote_bootstrap\" | sed \"s/__CMUX_WORKSPACE_ID__/$bmux_workspace_id/g; s/__CMUX_SURFACE_ID__/$bmux_surface_id/g\")\"",
            "bmux_remote_bootstrap_b64_runtime=\"$(printf '%s' \"$bmux_remote_bootstrap\" | base64 | tr -d '\\n')\"",
            "bmux_remote_command_template=\(shellQuote(remoteCommandTemplate))",
            "bmux_remote_command=\"$(printf '%s' \"$bmux_remote_command_template\" | sed \"s|\(remoteCommandBase64Placeholder)|$bmux_remote_bootstrap_b64_runtime|g\")\"",
        ]

        var sshInvocation = "command \(sshPrefix) -o \"RemoteCommand=$bmux_remote_command\""
        if !hasSSHOptionKey(options.sshOptions, key: "RequestTTY") {
            sshInvocation += " -tt"
        }
    ...
    private func runtimeEncodedRemoteBootstrapCommandShell(base64Placeholder: String) -> String {
        return [
            "bmux_tmp=$(mktemp \"${TMPDIR:-/tmp}/bmux-ssh-bootstrap.XXXXXX\") || exit 1",
            "(printf %s '\(base64Placeholder)' | base64 -d 2>/dev/null || printf %s '\(base64Placeholder)' | base64 -D 2>/dev/null) > \"$bmux_tmp\" || { rm -f \"$bmux_tmp\"; exit 1; }",
    ...
    func buildInteractiveRemoteShellScript(
        ...
        let shellStateDir = "$HOME/.bmux/relay/\(max(remoteRelayPort, 0)).shell"
        var commonShellLines = remoteTerminalLines
        commonShellLines.append(contentsOf: remoteEnvExportLines)
        commonShellLines.append("export PATH=\"$HOME/.bmux/bin:$PATH\"")
        if let relaySocket {
        ...
        var outerLines: [String] = [
            ...
            "    cat > \"$bmux_shell_dir/.zshenv\" <<'CMUXZSHENV'",
        ]
        outerLines.append(contentsOf: zshEnvLines)
        outerLines += [
            "CMUXZSHENV",
            "    cat > \"$bmux_shell_dir/.zprofile\" <<'CMUXZSHPROFILE'",
        ]
        outerLines.append(contentsOf: zshProfileLines)
        outerLines += [
            "CMUXZSHPROFILE",
            "    cat > \"$bmux_shell_dir/.zshrc\" <<'CMUXZSHRC'",
        ]
        outerLines.append(contentsOf: zshRCLines)
        outerLines += [
            "CMUXZSHRC",
            "    cat > \"$bmux_shell_dir/.zlogin\" <<'CMUXZSHLOGIN'",
        ]
        ...
        outerLines += [
            ...
            "    bmux_shell_dir=\"\(shellStateDir)\"",
            "    mkdir -p \"$bmux_shell_dir\"",
            "    cat > \"$bmux_shell_dir/.bashrc\" <<'CMUXBASHRC'",
        ]
        ...
        outerLines.append(contentsOf: relayWarmupLines.map { "    " + $0 })
        outerLines += [
            "    exec \"$CMUX_LOGIN_SHELL\" --rcfile \"$bmux_shell_dir/.bashrc\" -i",
            "    ;;",
            "  *)",
        ]
    ...
    private func interactiveRemoteTerminalSetupLines(terminfoSource: String?) -> [String] {
        ...
        lines += [
            "if [ \"$bmux_term\" != 'xterm-ghostty' ]; then",
            "  (",
    ...
    func encodedRemoteBootstrapCommand(_ remoteBootstrapScript: String) -> String {
        ...
        return [
            "bmux_tmp=$(mktemp \"${TMPDIR:-/tmp}/bmux-ssh-bootstrap.XXXXXX\") || exit 1",
            "(printf %s \(encodedLiteral) | base64 -d 2>/dev/null || printf %s \(encodedLiteral) | base64 -D 2>/dev/null) > \"$bmux_tmp\" || { rm -f \"$bmux_tmp\"; exit 1; }",
            "chmod 700 \"$bmux_tmp\" >/dev/null 2>&1 || true",
            "/bin/sh \"$bmux_tmp\"",
            "bmux_status=$?",
            "rm -f \"$bmux_tmp\"",
    ...
    func buildSSHStartupCommand(
        ...
        scriptLines += [
            ...
            "bmux_ssh_session_end() { if [ \"${CMUX_SSH_SESSION_ENDED:-0}\" = 1 ]; then return; fi; CMUX_SSH_SESSION_ENDED=1; \(lifecycleCleanup); }",
            "trap 'bmux_ssh_session_end' EXIT HUP INT TERM",
        ]
        if isShellSnippet {
    ...
    private func buildSSHSessionEndShellCommand(remoteRelayPort: Int) -> String {
        [
            ...
            "&& [ -n \"${CMUX_WORKSPACE_ID:-}\" ]",
            "&& [ -n \"${CMUX_SURFACE_ID:-}\" ]; then",
            "bmux ssh-session-end --relay-port \(remoteRelayPort) --workspace \"${CMUX_WORKSPACE_ID}\" --surface \"${CMUX_SURFACE_ID}\" >/dev/null 2>&1 || true;",
            "fi",
        ].joined(separator: " ")
    }
    ...
    private func runRemoteDaemonStatus(commandArgs: [String], jsonOutput: Bool) throws {
        ...
        let downloadCommand = "gh release download \(releaseTag) --repo manaflow-ai/bmux --pattern \(assetName)"
        let downloadChecksumsCommand = "gh release download \(releaseTag) --repo manaflow-ai/bmux --pattern \(checksumsAssetName)"
        let checksumVerifyCommand = "shasum -a 256 -c \(checksumsAssetName) --ignore-missing"
        let signerWorkflow = releaseTag == "nightly"
            ? "manaflow-ai/bmux/.github/workflows/nightly.yml"
            : "manaflow-ai/bmux/.github/workflows/release.yml"
        let verifyCommand = "gh attestation verify ./\(assetName) --repo manaflow-ai/bmux --signer-workflow \(signerWorkflow)"
    ...
    private func remoteDaemonCacheURL(version: String, goOS: String, goArch: String) -> URL {
        ...
        do {
            ...
            return URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
                .appendingPathComponent("bmux-remote-daemons", isDirectory: true)
    ...
    private func defaultSSHControlPathTemplate(remoteRelayPort: Int? = nil) -> String {
        if let remoteRelayPort, remoteRelayPort > 0 {
            return "/tmp/bmux-ssh-\(getuid())-\(remoteRelayPort)-%C"
        }
    ...
    private func cliDebugLog(_ message: @autoclosure () -> String) {
        ...
        let path: String? = {
            ...
            guard let marker = try? String(contentsOfFile: "/tmp/bmux-last-debug-log-path", encoding: .utf8) else {
                return nil
            }
        ...
        let line = "\(timestamp) [bmux-cli] \(message())\n"
    ...
    private func agentIntelRepoRoot(
        ...
        guard let client else {
            throw CLIError(message: "agent intel requires a live bmux session when using --session")
        }
    ...
    private func runAgentIntelCommand(
        commandArgs: [String],
        client: SocketClient?,
        jsonOutput: Bool,
        idFormat: CLIIDFormat
    ) throws {
        guard let intelSubcommandRaw = commandArgs.first else {
            throw CLIError(message: "agent intel requires a subcommand")
        }
        let intelSubcommand = intelSubcommandRaw.lowercased()
        let intelArgs = Array(commandArgs.dropFirst())
    ...
    private func runBrowserCommand(
        ...
        func displaySnapshotText(_ payload: [String: Any]) -> String {
            ...
            if url.isEmpty || url == "about:blank" {
                lines.append("hint: run 'bmux browser <surface> get url' to verify navigation")
            }
        ...
        func displayBrowserValue(_ value: Any) -> String {
            if let dict = value as? [String: Any],
               let type = dict["__bmux_t"] as? String,
               type == "undefined" {
                return "undefined"
            }
            if value is NSNull {
            ...
            if let outPathOpt {
                ...
                if !hasText(screenshotPath) && !hasText(screenshotURL) {
                    let outputDir = FileManager.default.temporaryDirectory
                        .appendingPathComponent("bmux-browser-screenshots-cli", isDirectory: true)
    ...
    private func subcommandUsage(_ command: String) -> String? {
        ...
        case "ping":
            ...
            Check connectivity to the bmux socket server.
            """
        case "capabilities":
            return """
            Usage: bmux capabilities
        ...
        case "welcome":
            ...
            Show a welcome screen with the bmux logo and useful shortcuts.
            Auto-runs once on first launch.
            """
        case "shortcuts":
            return """
            Usage: bmux shortcuts
        ...
        case "feedback":
            return """
            Usage: bmux feedback
                   bmux feedback --email <email> --body <text> [--image <path> ...]
        ...
        case "themes":
            ...
            Usage: bmux themes
                   bmux themes list
                   bmux themes set <theme>
                   bmux themes set --light <theme> [--dark <theme>]
                   bmux themes set --dark <theme> [--light <theme>]
                   bmux themes clear

            When run in a TTY, `bmux themes` opens an interactive theme picker with
            live app preview. Use `bmux themes list` for a plain listing.

            The picker previews the selected theme across the running bmux app and
            ...
            Commands:
              ...
              clear                     Remove the bmux theme override and fall back to other config

            Examples:
              bmux themes
              bmux themes list
              bmux themes set "Catppuccin Mocha"
              bmux themes set --light "Catppuccin Latte" --dark "Catppuccin Mocha"
              bmux themes clear
            """
        case "claude-teams":
            return String(localized: "cli.claude-teams.usage", defaultValue: """
            Usage: bmux claude-teams [claude-args...]

            Launch Claude Code with agent teams enabled.

            This command:
              - defaults Claude teammate mode to auto
              - sets a tmux-like environment so Claude auto mode uses bmux splits
            ...
            The tmux shim translates supported tmux window/pane commands into bmux
            workspace and split operations in the current bmux session.

            Examples:
              bmux claude-teams
              bmux claude-teams --continue
              bmux claude-teams --model sonnet
            """)
        case "omo":
            return String(localized: "cli.omo.usage", defaultValue: """
            Usage: bmux omo [opencode-args...]

            Launch OpenCode with oh-my-openagent in a bmux-aware environment.
            ...
            bmux splits with sidebar metadata and notifications.

            This command:
              - sets a tmux-like environment so oh-my-openagent uses bmux splits
              - prepends a private tmux shim to PATH
              - forwards all remaining arguments to opencode

            The tmux shim translates tmux window/pane commands into bmux workspace
            and split operations in the current bmux session.

            Examples:
              bmux omo
              bmux omo --continue
              bmux omo --model claude-sonnet-4-6
            """)
        case "identify":
            return """
            Usage: bmux identify [--workspace <id|ref|index>] [--surface <id|ref|index>] [--no-caller]
        ...
        case "list-windows":
            return """
            Usage: bmux list-windows

            List open windows.
            """
        case "current-window":
            return """
            Usage: bmux current-window

            Print the currently selected window ID.
            """
        case "new-window":
            return """
            Usage: bmux new-window
        ...
        case "focus-window":
            return """
            Usage: bmux focus-window --window <id|ref|index>
            ...
            Example:
              bmux focus-window --window 0
              bmux focus-window --window window:1
            """
        case "close-window":
            return """
            Usage: bmux close-window --window <id|ref|index>
            ...
            Example:
              bmux close-window --window 0
              bmux close-window --window window:1
            """
        case "move-workspace-to-window":
            return """
            Usage: bmux move-workspace-to-window --workspace <id|ref|index> --window <id|ref|index>
            ...
            Example:
              bmux move-workspace-to-window --workspace workspace:2 --window window:1
            """
        case "move-surface":
            return """
            Usage: bmux move-surface [--surface <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              bmux move-surface --surface surface:1 --workspace workspace:2
              bmux move-surface surface:1 --pane pane:2 --index 0
            """
        case "reorder-surface":
            return """
            Usage: bmux reorder-surface [--surface <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              bmux reorder-surface --surface surface:1 --index 0
              bmux reorder-surface --surface surface:3 --after surface:1
            """
        case "reorder-workspace":
            return """
            Usage: bmux reorder-workspace [--workspace <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              bmux reorder-workspace --workspace workspace:2 --index 0
              bmux reorder-workspace --workspace workspace:3 --after workspace:1
            """
        case "workspace-action":
            return """
            Usage: bmux workspace-action --action <name> [flags]
            ...
            Example:
              bmux workspace-action --workspace workspace:2 --action pin
              bmux workspace-action --action rename --title "infra"
              bmux workspace-action close-others
              bmux workspace-action --action set-color --color blue
              bmux workspace-action --action set-color --color "#C0392B"
              bmux workspace-action set-color Amber
              bmux workspace-action clear-color
            """
        case "tab-action":
            return """
            Usage: bmux tab-action --action <name> [flags]
            ...
            Example:
              bmux tab-action --tab tab:3 --action pin
              bmux tab-action --action close-right
              bmux tab-action --tab tab:2 --action rename --title "build logs"
            """
        case "rename-tab":
            return """
            Usage: bmux rename-tab [--workspace <id|ref>] [--tab <id|ref>] [--surface <id|ref>] [--] <title>
            ...
            Examples:
              bmux rename-tab "build logs"
              bmux rename-tab --tab tab:3 "staging server"
              bmux rename-tab --workspace workspace:2 --surface surface:5 --title "agent run"
            """
        case "new-workspace":
            return """
            Usage: bmux new-workspace [--name <title>] [--cwd <path>] [--command <text>]
            ...
            Example:
              bmux new-workspace
              bmux new-workspace --name "Build Server"
              bmux new-workspace --cwd ~/projects/myapp
              bmux new-workspace --cwd . --command "npm test"
            """
        case "list-workspaces":
            return """
            Usage: bmux list-workspaces
        ...
        case "ssh":
            return """
            Usage: bmux ssh <destination> [flags] [-- <remote-command-args>]

            Create a new workspace, mark it as remote-SSH, and start an SSH session in that workspace.
            bmux will also establish a local SSH proxy endpoint so browser traffic can egress from the remote host.
            ...
            Example:
              bmux ssh dev@my-host
              bmux ssh dev@my-host --name "gpu-box" --port 2222 --identity ~/.ssh/id_ed25519
              bmux ssh dev@my-host --ssh-option UserKnownHostsFile=/dev/null --ssh-option StrictHostKeyChecking=no
            """
        case "remote-daemon-status":
            return """
            Usage: bmux remote-daemon-status [--os <darwin|linux>] [--arch <arm64|amd64>]

            Show the embedded bmuxd-remote release manifest, local cache status, checksum verification state,
            and the GitHub attestation verification command for a target platform.

            Example:
              bmux remote-daemon-status
              bmux remote-daemon-status --os linux --arch arm64
            """
        case "new-split":
            return """
            Usage: bmux new-split <left|right|up|down> [flags]
            ...
            Example:
              bmux new-split right
              bmux new-split down --workspace workspace:1
            """
        case "list-panes":
            return """
            Usage: bmux list-panes [--workspace <id|ref>]
            ...
            Example:
              bmux list-panes
              bmux list-panes --workspace workspace:2
            """
        case "list-pane-surfaces":
            return """
            Usage: bmux list-pane-surfaces [--workspace <id|ref>] [--pane <id|ref>]
            ...
            Example:
              bmux list-pane-surfaces
              bmux list-pane-surfaces --workspace workspace:2 --pane pane:1
            """
        case "tree":
            return """
            Usage: bmux tree [flags]
            ...
            Output:
              ...
              - ◀ here (caller surface where `bmux tree` was invoked)
              - workspace [selected]
              - pane [focused]
              - surface [selected]
            ...
            Example:
              bmux tree
              bmux tree --all
              bmux tree --workspace workspace:2
              bmux --json tree --all
            """
        case "focus-pane":
            return """
            Usage: bmux focus-pane [--pane <id|ref> | <id|ref>] [flags]
            ...
            Example:
              ...
              bmux focus-pane pane:1
              bmux focus-pane --pane pane:1 --workspace workspace:2
            """
        case "new-pane":
            return """
            Usage: bmux new-pane [flags]
            ...
            Example:
              bmux new-pane
              bmux new-pane --type browser --direction down --url https://example.com
            """
        case "new-surface":
            return """
            Usage: bmux new-surface [flags]
        ...
        case "close-surface":
            return """
            Usage: bmux close-surface [flags]
            ...
            Example:
              bmux close-surface
              bmux close-surface --surface surface:3
            """
        case "drag-surface-to-split":
            return """
            Usage: bmux drag-surface-to-split --surface <id|ref> <left|right|up|down>
            ...
            Example:
              bmux drag-surface-to-split --surface surface:1 right
              bmux drag-surface-to-split --panel surface:2 down
            """
        case "refresh-surfaces":
            return """
            Usage: bmux refresh-surfaces

            Refresh surface snapshots for the focused workspace.
            """
        case "surface-health":
            return """
            Usage: bmux surface-health [--workspace <id|ref>]
        ...
        case "trigger-flash":
            return """
            Usage: bmux trigger-flash [--workspace <id|ref>] [--surface <id|ref>] [--panel <id|ref>]
            ...
            Example:
              bmux trigger-flash
              bmux trigger-flash --workspace workspace:2 --surface surface:3
            """
        case "list-panels":
            return """
            Usage: bmux list-panels [--workspace <id|ref>]
            ...
            Example:
              bmux list-panels
              bmux list-panels --workspace workspace:2
            """
        case "focus-panel":
            return """
            Usage: bmux focus-panel --panel <id|ref> [--workspace <id|ref>]
            ...
            Example:
              bmux focus-panel --panel surface:2
              bmux focus-panel --panel surface:5 --workspace workspace:2
            """
        case "close-workspace":
            return """
            Usage: bmux close-workspace --workspace <id|ref|index>
        ...
        case "select-workspace":
            return """
            Usage: bmux select-workspace --workspace <id|ref|index>
            ...
            Example:
              ...
              bmux select-workspace --workspace 0
            """
        case "rename-workspace", "rename-window":
            return """
            Usage: bmux rename-workspace [--workspace <id|ref|index>] [--] <title>
            ...
            Example:
              bmux rename-workspace "backend logs"
              bmux rename-window --workspace workspace:2 "agent run"
            """
        case "current-workspace":
            return """
            Usage: bmux current-workspace

            Print the currently selected workspace ID.
            """
        case "capture-pane":
            return """
            Usage: bmux capture-pane [--workspace <id|ref>] [--surface <id|ref>] [--scrollback] [--lines <n>]
            ...
            Example:
              bmux capture-pane --workspace workspace:2 --surface surface:1 --scrollback --lines 200
            """
        case "resize-pane":
            return """
            Usage: bmux resize-pane [--pane <id|ref>] [--workspace <id|ref>] [-L|-R|-U|-D] [--amount <n>]
        ...
        case "pipe-pane":
            return """
            Usage: bmux pipe-pane [--workspace <id|ref>] [--surface <id|ref>] [--command <shell-command> | <shell-command>]
        ...
        case "wait-for":
            return """
            Usage: bmux wait-for [-S|--signal] <name> [--timeout <seconds>]
        ...
        case "swap-pane":
            return """
            Usage: bmux swap-pane --pane <id|ref> --target-pane <id|ref> [--workspace <id|ref>]
        ...
        case "break-pane":
            return """
            Usage: bmux break-pane [--workspace <id|ref>] [--pane <id|ref>] [--surface <id|ref>] [--no-focus]
        ...
        case "join-pane":
            return """
            Usage: bmux join-pane --target-pane <id|ref> [--workspace <id|ref>] [--pane <id|ref>] [--surface <id|ref>] [--no-focus]
        ...
        case "last-pane":
            return """
            Usage: bmux last-pane [--workspace <id|ref>]
        ...
        case "find-window":
            return """
            Usage: bmux find-window [--content] [--select] [query]
        ...
        case "clear-history":
            return """
            Usage: bmux clear-history [--workspace <id|ref>] [--surface <id|ref>]
        ...
        case "set-hook":
            return """
            Usage: bmux set-hook [--list] [--unset <event>] | <event> <command>
        ...
        case "set-buffer":
            return """
            Usage: bmux set-buffer [--name <name>] [--] <text>
        ...
        case "paste-buffer":
            return """
            Usage: bmux paste-buffer [--name <name>] [--workspace <id|ref>] [--surface <id|ref>]
        ...
        case "list-buffers":
            return """
            Usage: bmux list-buffers

            List tmux-compat buffers.
            """
        case "respawn-pane":
            return """
            Usage: bmux respawn-pane [--workspace <id|ref>] [--surface <id|ref>] [--command <cmd> | <cmd>]
        ...
        case "display-message":
            return """
            Usage: bmux display-message [-p|--print] <text>
        ...
        case "read-screen":
            ...
            Example:
              bmux read-screen
              bmux read-screen --surface surface:2 --scrollback --lines 200
            """
        case "send":
            return """
            Usage: bmux send [flags] [--] <text>
            ...
            Example:
              ...
              bmux send --surface surface:2 "ls -la\\n"
            """
        case "send-key":
            return """
            Usage: bmux send-key [flags] [--] <key>
            ...
            Example:
              bmux send-key enter
              bmux send-key --surface surface:2 ctrl+c
            """
        case "send-panel":
            return """
            Usage: bmux send-panel --panel <id|ref> [flags] [--] <text>
            ...
            Example:
              bmux send-panel --panel surface:2 "echo hello\\n"
            """
        case "send-key-panel":
            return """
            Usage: bmux send-key-panel --panel <id|ref> [flags] [--] <key>
            ...
            Example:
              bmux send-key-panel --panel surface:2 enter
              bmux send-key-panel --panel surface:2 ctrl+c
            """
        case "notify":
            return """
            Usage: bmux notify [flags]
            ...
            Example:
              bmux notify --title "Build done" --body "All tests passed"
              bmux notify --title "Error" --subtitle "test.swift" --body "Line 42: syntax error"
            """
        case "list-notifications":
            return """
            Usage: bmux list-notifications

            List queued notifications.
            """
        case "clear-notifications":
            return """
            Usage: bmux clear-notifications

            Clear all queued notifications.
            """
        case "set-status":
            return """
            Usage: bmux set-status <key> <value> [flags]
            ...
            Example:
              bmux set-status build "compiling" --icon hammer --color "#ff9500"
              bmux set-status deploy "v1.2.3" --workspace workspace:2
            """
        case "clear-status":
            return """
            Usage: bmux clear-status <key> [flags]
            ...
            Example:
              bmux clear-status build
            """
        case "list-status":
            return """
            Usage: bmux list-status [flags]
            ...
            Example:
              bmux list-status
              bmux list-status --workspace workspace:2
            """
        case "set-progress":
            return """
            Usage: bmux set-progress <0.0-1.0> [flags]
            ...
            Example:
              bmux set-progress 0.5 --label "Building..."
              bmux set-progress 1.0 --label "Done"
            """
        case "clear-progress":
            return """
            Usage: bmux clear-progress [flags]
        ...
        case "sidebar-state":
            return """
            Usage: bmux sidebar-state [flags]
        ...
        case "set-app-focus":
            return """
            Usage: bmux set-app-focus <active|inactive|clear>

            Override app focus state for notification routing tests.

            Example:
              bmux set-app-focus inactive
              bmux set-app-focus clear
            """
        case "simulate-app-active":
            return """
            Usage: bmux simulate-app-active

            Trigger the app-active handler used by notification focus tests.
            """
        case "claude-hook":
            return """
            Usage: bmux claude-hook <session-start|active|stop|idle|notification|notify|prompt-submit> [flags]
            ...
            Example:
              echo '{"session_id":"abc"}' | bmux claude-hook session-start
        ...
        case "codex-hook":
            return """
            Usage: bmux codex-hook <session-start|prompt-submit|stop> [flags]

            Hook for Codex CLI integration. Reads JSON from stdin.
            Gracefully no-ops when not running inside bmux.
        ...
        case "browser":
            ...
            Example:
              ...
              bmux browser surface:1 navigate https://google.com
              bmux browser --surface surface:1 snapshot --interactive
            """
        // Legacy browser aliases — point users to `bmux browser --help`
        case "open-browser":
            return "Legacy alias for 'bmux browser open'. Run 'bmux browser --help' for details."
        case "navigate":
            return "Legacy alias for 'bmux browser navigate'. Run 'bmux browser --help' for details."
        case "browser-back":
            return "Legacy alias for 'bmux browser back'. Run 'bmux browser --help' for details."
        case "browser-forward":
            return "Legacy alias for 'bmux browser forward'. Run 'bmux browser --help' for details."
        case "browser-reload":
            return "Legacy alias for 'bmux browser reload'. Run 'bmux browser --help' for details."
        case "get-url":
            return "Legacy alias for 'bmux browser get-url'. Run 'bmux browser --help' for details."
        case "focus-webview":
            return "Legacy alias for 'bmux browser focus-webview'. Run 'bmux browser --help' for details."
        case "is-webview-focused":
            return "Legacy alias for 'bmux browser is-webview-focused'. Run 'bmux browser --help' for details."
        case "markdown":
            return """
            Usage: bmux markdown open <path> [options]
                   bmux markdown <path>       (shorthand for 'open')
            ...
            Examples:
              ...
              bmux markdown ~/project/CHANGELOG.md
              bmux markdown open ./docs/design.md --workspace 0
              bmux markdown open plan.md --direction down
            """
        default:
            return nil
        }
    ...
    private static let bmuxThemeOverrideBundleIdentifier = "com.bmuxterm.app"
    private static let bmuxThemesBlockStart = "# bmux themes start"
    private static let bmuxThemesBlockEnd = "# bmux themes end"
    private static let bmuxThemesReloadNotificationName = "com.bmuxterm.themes.reload-config"
    ...
    private func runInteractiveThemes() throws {
        ...
        environment["CMUX_THEME_PICKER_CONFIG"] = try bmuxThemeOverrideConfigURL().path
        environment["CMUX_THEME_PICKER_BUNDLE_ID"] = currentBmuxAppBundleIdentifier() ?? Self.bmuxThemeOverrideBundleIdentifier
    ...
    private func printThemesList(jsonOutput: Bool) throws {
        ...
        let configPath = try bmuxThemeOverrideConfigURL().path
    ...
    private func themeConfigSearchURLs() -> [URL] {
        let rawPaths = [
            ...
            "~/Library/Application Support/\(Self.bmuxThemeOverrideBundleIdentifier)/config",
            "~/Library/Application Support/\(Self.bmuxThemeOverrideBundleIdentifier)/config.ghostty",
        ]
    ...
    private func bmuxThemeOverrideConfigURL() throws -> URL {
        guard let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first else {
    ...
    private func writeManagedThemeOverride(rawThemeValue: String) throws -> URL {
        let fileManager = FileManager.default
        let configURL = try bmuxThemeOverrideConfigURL()
    ...
    private func reloadThemesIfPossible() -> ThemeReloadStatus {
        let bundleIdentifier = currentBmuxAppBundleIdentifier() ?? Self.bmuxThemeOverrideBundleIdentifier
    ...
    private func currentBmuxAppBundleIdentifier() -> String? {
    ...
    private func isBmuxClaudeWrapper(at path: String) -> Bool {
    ...
    private func resolveClaudeExecutable(searchPath: String?) -> String? {
        resolveExecutableInSearchPath(
            ...
            skip: { self.isBmuxClaudeWrapper(at: $0) }
        )
    }
    ...
    private func configureTmuxCompatEnvironment(
        ...
        setenv(bmuxBinEnvVar, executablePath, 1)
    ...
    private func configureClaudeTeamsEnvironment(
        ...
        configureTmuxCompatEnvironment(
            ...
            bmuxBinEnvVar: "CMUX_CLAUDE_TEAMS_CMUX_BIN",
    ...
    private func createClaudeTeamsShimDirectory() throws -> URL {
        ...
        #!/usr/bin/env bash
        set -euo pipefail
        exec "${CMUX_CLAUDE_TEAMS_CMUX_BIN:-bmux}" __tmux-compat "$@"
        """
    ...
    private func createClaudeNodeOptionsRestoreModule() throws -> URL {
        let root = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("bmux-claude-node-options", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true, attributes: nil)
    ...
    // MARK: - bmux omo (OpenCode + oh-my-openagent)
    ...
    private func createOMOShimDirectory() throws -> URL {
        // tmux shim: redirects tmux commands to bmux __tmux-compat
        ...
        exec "${CMUX_OMO_CMUX_BIN:-bmux}" __tmux-compat "$@"
        """
        ...
        // terminal-notifier shim: intercepts macOS notifications and routes to bmux notify
        ...
        exec "${CMUX_OMO_CMUX_BIN:-bmux}" notify --title "${TITLE:-OpenCode}" --body "${BODY:-}"
        """
    ...
    private func configureOMOEnvironment(
        ...
        configureTmuxCompatEnvironment(
            ...
            bmuxBinEnvVar: "CMUX_OMO_CMUX_BIN",
    ...
    private func runClaudeTeamsTmuxCompat(
        ...
        case "new-session", "new":
            ...
            if parsed.hasFlag("-A") {
                throw CLIError(message: "new-session -A is not supported in bmux claude-teams mode")
            }
        ...
        case "new-window", "neww":
            ...
            if parsed.value("-t") != nil {
                throw CLIError(message: "new-window -t is not supported in bmux claude-teams mode")
            }
            var params: [String: Any] = ["focus": false]
            if let cwd = parsed.value("-c") {
                params["cwd"] = resolvePath(cwd)
    ...
    private func tmuxCompatStoreURL() -> URL {
        ...
        return URL(fileURLWithPath: homePath)
            .appendingPathComponent(".bmuxterm")
    ...
    private func tmuxWaitForSignalURL(name: String) -> URL {
        ...
        return URL(fileURLWithPath: "/tmp/bmux-wait-for-\(String(sanitized)).sig")
    }
    ...
    private func runTmuxCompatCommand(
        ...
        case "popup":
            throw CLIError(message: "popup is not supported yet in bmux CLI parity mode")

        case "bind-key", "unbind-key", "copy-mode":
            throw CLIError(message: "\(command) is not supported yet in bmux CLI parity mode")
        ...
        case "display-message":
            ...
            let payload = try client.sendV2(method: "notification.create", params: ["title": "bmux", "body": message])
            if jsonOutput {
                print(jsonString(payload))
            } else {
                print(message)
    ...
    private func runClaudeHook(
        ...
        case "help", "--help", "-h":
            ...
            print(
                """
                bmux claude-hook <session-start|stop|session-end|notification|prompt-submit|pre-tool-use> [--workspace <id|index>] [--surface <id|index>]
                """
            )
    ...
    private func codexHookTranscriptPath(
        sessionId: String?,
        ...
        for candidate in [transcriptPath, sessionRecord?.transcriptPath] {
            guard let normalized = agentIntelNormalizedPath(candidate),
        ...
        guard let sessionId = sessionId?.trimmingCharacters(in: .whitespacesAndNewlines),
              !sessionId.isEmpty,
              let sessionsRoot = agentIntelNormalizedPath((processEnv["CODEX_HOME"] ?? "~/.codex") + "/sessions"),
              fileManager.fileExists(atPath: sessionsRoot),
              let enumerator = fileManager.enumerator(
                  at: URL(fileURLWithPath: sessionsRoot, isDirectory: true),
        ...
        var bestMatch: (path: String, modifiedAt: Date)?
        for case let fileURL as URL in enumerator {
            ...
            bestMatch = (fileURL.path, modifiedAt)
        }

        return bestMatch?.path
    }

    private func codexHookTranscriptCompactionCount(path: String) -> Int? {
        ...
        content.enumerateLines { line, _ in
            guard line.contains("context_compacted"),
                  let lineData = line.data(using: .utf8),
                  let object = try? JSONSerialization.jsonObject(with: lineData) as? [String: Any],
                  (object["type"] as? String) == "event_msg",
                  let payload = object["payload"] as? [String: Any],
                  (payload["type"] as? String) == "context_compacted" else {
                return
            }
            count += 1
        }
    ...
    private func codexHookCompactionSnapshot(
        sessionId: String?,
        transcriptPath: String?,
        sessionRecord: ClaudeHookSessionRecord?,
        processEnv: [String: String]
    ) -> CodexHookCompactionSnapshot? {
        guard let transcriptPath = codexHookTranscriptPath(
            sessionId: sessionId,
            transcriptPath: transcriptPath,
            sessionRecord: sessionRecord,
            processEnv: processEnv
        ),
        let compactionCount = codexHookTranscriptCompactionCount(path: transcriptPath) else {
            return nil
        }

        return CodexHookCompactionSnapshot(
            transcriptPath: transcriptPath,
            compactionCount: compactionCount
        )
    }

    private func codexHookShouldRefreshAfterCompaction(
        snapshot: CodexHookCompactionSnapshot?,
        sessionRecord: ClaudeHookSessionRecord?
    ) -> Bool {
        guard let snapshot,
              let sessionRecord else {
        ...
        return snapshot.compactionCount > (sessionRecord.observedCompactionCount ?? 0)
    }
    ...
    private func runCodexInstallHooks() throws {
        ...
        if !hooksChanged && !configChanged {
            print("bmux hooks are already installed. Nothing to change.")
            return
        }
        ...
        print("")
        print("Installed. Hooks activate inside bmux and silently no-op elsewhere.")
        print("To remove: bmux codex uninstall-hooks")
    }
    ...
    private func runCodexUninstallHooks() throws {
        ...
        print("Removed bmux Codex hooks.")
    }
    ...
    private func codexHookRefreshContextLabel(
        instructionsChanged: Bool,
        compactionChanged: Bool
    ) -> String {
        switch (instructionsChanged, compactionChanged) {
        case (true, true):
            return "instructions, skills, or compacted context"
        case (true, false):
            return "updated instructions or skills"
        case (false, true):
    ...
    private func codexHookRefreshNotificationSubtitle(
        replayed: Bool,
        instructionsChanged: Bool,
        compactionChanged: Bool
    ) -> String {
        if instructionsChanged && !compactionChanged {
            return replayed ? "Instructions refreshed" : "Instructions updated"
        }
        return replayed ? "Context refreshed" : "Context refresh needed"
    }
    ...
    private func codexHookRefreshNotificationBody(
        ...
        let refreshContext = codexHookRefreshContextLabel(
            instructionsChanged: instructionsChanged,
            compactionChanged: compactionChanged
        )
        let trackedCount = snapshot?.trackedPaths.count ?? 0
        if replayed {
            return trackedCount > 0
                ? "Detected \(refreshContext) and replayed the prompt with the refreshed context (\(trackedCount) tracked files)."
                : "Detected \(refreshContext) and replayed the prompt with the refreshed context."
        }
        return trackedCount > 0
            ? "Detected \(refreshContext). Re-send the prompt to continue with the refreshed context (\(trackedCount) tracked files)."
            : "Detected \(refreshContext). Re-send the prompt to continue with the refreshed context."
    }

    private func codexHookReplayPrompt(for originalPrompt: String) -> String {
        "\(Self.codexHookReplayPrefix) Re-read the latest repo and global instructions from disk, then reopen any skills relevant to this request before answering. Then continue with the original user request: \(originalPrompt)"
    }
    ...
    private func runCodexHook(
        ...
        case "session-start":
            ...
            let compactionSnapshot = codexHookCompactionSnapshot(
                sessionId: parsedInput.sessionId,
            ...
            if let sessionId = parsedInput.sessionId {
                try? sessionStore.upsert(
                    ...
                    surfaceId: surfaceId,
                    cwd: parsedInput.cwd,
                    transcriptPath: compactionSnapshot?.transcriptPath ?? parsedInput.transcriptPath,
                    instructionFingerprint: instructionSnapshot?.fingerprint,
                    observedCompactionCount: compactionSnapshot?.compactionCount
                )
            }
        ...
        case "prompt-submit":
            ...
            let instructionsChanged = codexHookShouldRefreshInstructions(
                snapshot: instructionSnapshot,
                sessionRecord: mappedSession
            )
            let compactionChanged = codexHookShouldRefreshAfterCompaction(
                snapshot: compactionSnapshot,
                sessionRecord: mappedSession
            )
            let shouldRefresh = instructionsChanged || compactionChanged
            if shouldRefresh {
                telemetry.breadcrumb(
                    "codex-hook.prompt-submit.refresh",
                    data: [
                        "tracked_files": instructionSnapshot?.trackedPaths.count ?? 0,
                        "instructions_changed": instructionsChanged,
                        "compaction_changed": compactionChanged,
                        "observed_compactions": compactionSnapshot?.compactionCount ?? 0
                    ]
                )
                ...
                _ = try? client.sendV2(
                    ...
                    params: [
                        ...
                        "surface_id": surfaceId,
                        "title": "Codex",
                        "subtitle": codexHookRefreshNotificationSubtitle(
                            replayed: replayed,
                            instructionsChanged: instructionsChanged,
                            compactionChanged: compactionChanged
                        ),
                        "body": sanitizeNotificationField(
                            codexHookRefreshNotificationBody(
                                snapshot: instructionSnapshot,
                                replayed: replayed,
                                instructionsChanged: instructionsChanged,
                                compactionChanged: compactionChanged
                ...
                let reason = replayed
                    ? "bmux detected \(refreshContext), refreshed the session, and replayed your prompt once."
                    : "bmux detected \(refreshContext). Re-send your prompt to continue with the refreshed context."
        ...
        case "help", "--help", "-h":
            print("bmux codex-hook <session-start|prompt-submit|stop> [--workspace <id>] [--surface <id>]")
    ...
    private func versionSummary() -> String {
        ...
        if let version = info["CFBundleShortVersionString"], let build = info["CFBundleVersion"] {
            baseSummary = "bmux \(version) (\(build))"
        } else if let version = info["CFBundleShortVersionString"] {
            baseSummary = "bmux \(version)"
        } else if let build = info["CFBundleVersion"] {
            baseSummary = "bmux build \(build)"
        } else {
            baseSummary = "bmux version unknown"
        }
    ...
    private func printWelcome() {
        ...
        print("  \(bold)GitHub\(reset)\(subdued)              https://github.com/manaflow-ai/bmux (please leave a star ⭐)\(reset)")
        print("  \(bold)Email\(reset)\(subdued)               founders@manaflow.com\(reset)")
        print()
        print("  \(subdued)Run \(reset)\(bold)bmux --help\(reset)\(subdued) for all commands.\(reset)")
        print("  \(subdued)Run \(reset)\(bold)bmux shortcuts\(reset)\(subdued) to edit shortcuts.\(reset)")
        print("  \(subdued)Run \(reset)\(bold)bmux feedback\(reset)\(subdued) to report a bug.\(reset)")
        print()
    }
    ...
    private func usage() -> String {
        return """
        bmux - control bmux via Unix socket

        Usage:
          bmux <path>                Open a directory in a new workspace (launches bmux if needed)
          bmux [global-options] <command> [options]
        ...
        Environment:
          CMUX_WORKSPACE_ID   Auto-set in bmux terminals. Used as default --workspace for
