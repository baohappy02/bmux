...
extension Notification.Name {
    ...
    static let terminalSurfaceDidBecomeReady = Notification.Name("bmux.terminalSurfaceDidBecomeReady")
...
class TerminalController {
    ...
    private static let v2BrowserEvalEnvelopeTypeKey = "__bmux_t"
    private static let v2BrowserEvalEnvelopeValueKey = "__bmux_v"
    ...
    private struct AgentProfileJobSpec {
        let label: String
        let command: String
        let split: String?
    }

    private struct AgentProfileSpec {
        let name: String
    ...
    private func withV2AgentState<T>(_ body: () -> T) -> T {
        v2AgentStateLock.lock()
        defer { v2AgentStateLock.unlock() }
        return body()
    }
    ...
    private final class SocketFastPathState: @unchecked Sendable {
        private let queue = DispatchQueue(label: "com.bmux.socket-fast-path")
        ...
        if accessMode == .bmuxOnly {
            ...
            if let pid {
                guard isDescendant(pid) else {
                    let msg = "ERROR: Access denied — only processes started inside bmux can connect\n"
    ...
    private func processV2Command(_ jsonLine: String) -> String {
...
#if DEBUG
        if method.hasPrefix("agent.") || method.hasPrefix("browser.") {
            dlog("v2.request method=\(method) paramsKeys=\(params.keys.sorted().joined(separator: ","))")
        }
#endif
        ...
        case "agent.ensure":
            return v2Result(id: id, self.v2AgentEnsure(params: params))
        case "agent.batch":
            return v2Result(id: id, self.v2AgentBatch(params: params))
        case "agent.focus":
        ...
        case "agent.terminal.write":
            return v2Result(id: id, self.v2AgentTerminalWrite(params: params))
        case "agent.terminal.capture":
            return v2Result(id: id, self.v2AgentTerminalCapture(params: params))
        case "agent.terminal.wait":
            return v2Result(id: id, self.v2AgentTerminalWait(params: params))
        case "agent.task.run":
            return v2Result(id: id, self.v2AgentTaskRun(params: params))
        case "agent.task.logs":
            return v2Result(id: id, self.v2AgentTaskLogs(params: params))
        case "agent.task.run_profile":
            return v2Result(id: id, self.v2AgentTaskRunProfile(params: params))
        case "agent.task.run_many":
            return v2Result(id: id, self.v2AgentTaskRunMany(params: params))
        case "agent.task.cancel":
            return v2Result(id: id, self.v2AgentTaskCancel(params: params))
        case "agent.task.wait":
    ...
    private func v2AgentCapabilities(params: [String: Any]) -> V2CallResult {
        let session = v2String(params, "session_id").flatMap { sessionId in
            withV2AgentState { v2AgentSessions[sessionId] }
        }
#if DEBUG
        dlog("agent.capabilities.start session=\(session?.id ?? "nil")")
#endif
        guard let context = v2AgentResolveContext(params: params, session: session) else {
            return .err(code: "not_found", message: "Workspace not found", data: nil)
        }
#if DEBUG
        dlog("agent.capabilities.after_context workspace=\(context.workspace.id.uuidString.prefix(5)) surface=\(context.surfaceId?.uuidString.prefix(5) ?? "nil")")
#endif
...
#if DEBUG
        dlog("agent.capabilities.after_package_manager manager=\(packageManager ?? "nil")")
#endif
    ...
    private func v2AgentEnsure(params: [String: Any]) -> V2CallResult {
        ...
        case "browser":
            ...
            let existingBrowserId = v2AgentReusableBrowserSurfaceId(
                params: params,
                context: context,
                requestedURL: requestedURL
            )
            if let existingBrowserId {
                let browserContext = AgentWorkspaceContext(
                    windowId: context.windowId,
                    tabManager: context.tabManager,
                    workspace: context.workspace,
                    paneId: context.workspace.paneId(forPanelId: existingBrowserId)?.id,
                    surfaceId: existingBrowserId
                )
                let updatedSession = v2AgentPersistSession(
                    sessionId: sessionId,
                    session: session,
                ...
                var payload = v2AgentSurfaceSummary(context: browserContext)
                payload["session_id"] = sessionId
        ...
        case "service":
            guard let port = v2Int(params, "port"), port > 0, port <= 65535 else {
                return .err(code: "invalid_params", message: "Service ensure requires port", data: nil)
            }
            let services = v2AgentServicesPayload(workspace: context.workspace, sessionId: sessionId)
            if let service = services.first(where: { v2StrictIntAny($0["port"]) == port }) {
                var payload = service
            ...
            guard let profileName = v2String(params, "profile")?.trimmingCharacters(in: .whitespacesAndNewlines),
                  !profileName.isEmpty,
                  v2AgentResolveProfile(name: profileName, context: context) != nil else {
                return .err(code: "not_found", message: "No matching service and no runnable profile provided", data: [
                    "port": port,
                    "session_id": sessionId
                ])
            }

            let runningTasks = v2AgentTasks.values.filter { task in
                guard task.sessionId == sessionId,
                      task.workspaceId == context.workspace.id,
                      !task.status.isTerminal else {
                    return false
                }
                return v2AgentTask(task, matchesServicePort: port, in: context.workspace)
            }
            if !runningTasks.isEmpty {
                return .ok([
                    ...
                    "port": port,
                    "job_ids": runningTasks.map(\.id),
                    "profile": profileName,
                    "expected_ports": Array(Set(runningTasks.flatMap(\.expectedPorts))).sorted(),
                    "event_cursor": v2AgentCurrentEventCursor()
                ])
            }

            var profileParams = params
            profileParams["profile"] = profileName
            switch v2AgentTaskRunProfile(params: profileParams) {
            case .ok(let value):
                var payload = (value as? [String: Any]) ?? [:]
                payload["target"] = "service"
                payload["created"] = true
        ...
        case "profile":
            switch v2AgentTaskRunProfile(params: params) {
            case .ok(let value):
                var payload = (value as? [String: Any]) ?? [:]
        ...
        default:
            return .err(code: "invalid_params", message: "Unsupported ensure target", data: [
                "target": target
            ])
        }
    }

    private func v2AgentBatch(params: [String: Any]) -> V2CallResult {
        guard let sessionId = v2String(params, "session_id") else {
            return .err(code: "invalid_params", message: "Missing session_id", data: nil)
        }
        ...
        guard let steps = params["steps"] as? [[String: Any]], !steps.isEmpty else {
            return .err(code: "invalid_params", message: "Missing steps", data: nil)
        }

        let continueOnError = v2Bool(params, "continue_on_error") ?? false
        var stepResults: [[String: Any]] = []
        var overallOK = true

        for (index, step) in steps.enumerated() {
            guard let kind = (step["kind"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines),
                  !kind.isEmpty else {
                overallOK = false
                stepResults.append([
                    "index": index,
                    "ok": false,
                    "code": "invalid_params",
                    "message": "Batch step missing kind"
                ])
            ...
            var merged = params
            merged.removeValue(forKey: "steps")
            merged.removeValue(forKey: "continue_on_error")
            for (key, value) in step where key != "kind" {
                merged[key] = value
            }

            let callResult: V2CallResult
            switch kind {
            case "ensure.terminal":
                merged["target"] = "terminal"
                callResult = v2AgentEnsure(params: merged)
            case "ensure.browser":
                merged["target"] = "browser"
                callResult = v2AgentEnsure(params: merged)
            case "ensure.service":
                merged["target"] = "service"
                callResult = v2AgentEnsure(params: merged)
            case "ensure.profile":
                merged["target"] = "profile"
                callResult = v2AgentEnsure(params: merged)
            case "task.run_profile", "run_profile":
                callResult = v2AgentTaskRunProfile(params: merged)
            case "service.wait":
                callResult = v2AgentServiceWait(params: merged)
            case "service.list":
                callResult = v2AgentServiceList(params: merged)
            default:
                callResult = .err(code: "invalid_params", message: "Unsupported batch step kind", data: ["kind": kind])
            }

            switch callResult {
            case .ok(let value):
                var entry: [String: Any] = ["index": index, "kind": kind, "ok": true]
    ...
    private func v2AgentTerminalWrite(params: [String: Any]) -> V2CallResult {
        guard let sessionId = v2String(params, "session_id") else {
            return .err(code: "invalid_params", message: "Missing session_id", data: nil)
        }
        guard let session = v2AgentSessions[sessionId] else {
        ...
        guard let text = v2String(params, "text"), !text.isEmpty else {
            return .err(code: "invalid_params", message: "Missing text", data: nil)
        }

        var result: V2CallResult = .err(code: "internal_error", message: "Failed to write terminal input", data: nil)
        v2MainSync {
            guard let resolved = v2AgentResolveTerminalTarget(params: params, session: session) else {
                result = .err(code: "invalid_params", message: "Target surface is not a terminal", data: ["session_id": sessionId])
                return
            }

            let queued = v2AgentSendText(text, terminalPanel: resolved.terminalPanel)
            let updatedSession = v2AgentPersistSession(
                sessionId: sessionId,
                session: session,
            ...
            var payload = v2AgentSurfaceSummary(context: resolved.context)
            payload["session_id"] = sessionId
            payload["layout_rev"] = updatedSession.layoutRev
            payload["queued"] = queued
            payload["chars"] = text.count
            result = .ok(payload)
        }
        return result
    }

    private func v2AgentTerminalCapture(params: [String: Any]) -> V2CallResult {
        guard let sessionId = v2String(params, "session_id") else {
            return .err(code: "invalid_params", message: "Missing session_id", data: nil)
        }
        guard var session = v2AgentSessions[sessionId] else {
            return .err(code: "not_found", message: "Agent session not found", data: ["session_id": sessionId])
        }

        let mode = (v2String(params, "mode") ?? "tail").trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        if !["tail", "full", "delta"].contains(mode) {
            return .err(code: "invalid_params", message: "Unsupported terminal capture mode", data: [
                "mode": mode,
                "supported_modes": ["tail", "full", "delta"]
            ])
        }
        var includeScrollback = v2Bool(params, "scrollback") ?? true
        let lineLimit: Int? = {
            if let requested = v2Int(params, "lines") {
                return requested
            }
            return mode == "full" ? nil : 80
        }()
        ...
        var result: V2CallResult = .err(code: "internal_error", message: "Failed to capture terminal text", data: nil)
        v2MainSync {
            guard let resolved = v2AgentResolveTerminalTarget(params: params, session: session) else {
                result = .err(code: "invalid_params", message: "Target surface is not a terminal", data: ["session_id": sessionId])
                return
            ...
            let response = readTerminalTextBase64(
                terminalPanel: resolved.terminalPanel,
            ...
            let fullText = Data(base64Encoded: base64).flatMap { String(data: $0, encoding: .utf8) } ?? ""
            let previousBaseline = resolved.context.surfaceId.flatMap { session.terminalCaptureBaselineBySurface[$0] }
            let text: String
            switch mode {
            case "delta":
                if let previousBaseline, fullText.hasPrefix(previousBaseline) {
                    text = String(fullText.dropFirst(previousBaseline.count))
                } else {
                    text = fullText
                }
            case "full":
            ...
            if let surfaceId = resolved.context.surfaceId {
                session.terminalCaptureBaselineBySurface[surfaceId] = fullText
            }
            let updatedSession = v2AgentPersistSession(
                sessionId: sessionId,
                session: session,
            ...
            var storedSession = updatedSession
            storedSession.terminalCaptureBaselineBySurface = session.terminalCaptureBaselineBySurface
            v2AgentSessions[sessionId] = storedSession
    ...
    private func v2AgentTerminalWait(params: [String: Any]) -> V2CallResult {
        guard let sessionId = v2String(params, "session_id") else {
            return .err(code: "invalid_params", message: "Missing session_id", data: nil)
        }
        guard let session = v2AgentSessions[sessionId] else {
        ...
        let requestedState = (v2String(params, "state") ?? "prompt").trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        if !["prompt", "running"].contains(requestedState) {
            return .err(code: "invalid_params", message: "Unsupported wait state", data: [
                "state": requestedState,
                "supported_states": ["prompt", "running"]
            ])
        }
        let timeoutMs = max(0, v2Int(params, "timeout_ms") ?? 30_000)
        ...
        while true {
            ...
            v2MainSync {
                guard let resolved = v2AgentResolveTerminalTarget(params: params, session: session) else {
                    waitResult = .err(code: "invalid_params", message: "Target surface is not a terminal", data: ["session_id": sessionId])
                    return
                }

                let shellState = resolved.context.workspace.panelShellActivityState(panelId: resolved.terminalPanel.id)
                ...
                if Date() >= deadline {
                    ...
                    payload["session_id"] = sessionId
                    payload["state"] = stateText
                    payload["expected_state"] = requestedState
                    waitResult = .err(code: "timeout", message: "Timed out waiting for terminal state", data: payload)
                    return
                }
            }
    ...
    private func v2AgentTaskRun(params: [String: Any]) -> V2CallResult {
        ...
        let label: String
        if let rawLabel = v2String(params, "label")?.trimmingCharacters(in: .whitespacesAndNewlines),
           !rawLabel.isEmpty {
            label = rawLabel
        } else {
            label = v2AgentDefaultTaskLabel(for: command)
        }
        let cwd = v2String(params, "cwd")?.trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    private func v2AgentTaskRunProfile(params: [String: Any]) -> V2CallResult {
        ...
        guard let profile = v2AgentResolveProfile(name: profileName, context: context) else {
            return .err(code: "not_found", message: "Profile not available", data: [
                "session_id": sessionId,
                "profile": profileName
            ])
        }
        guard !profile.jobs.isEmpty else {
            return .err(code: "invalid_state", message: "Profile has no runnable jobs", data: [
                "session_id": sessionId,
                "profile": profileName
            ])
        }
        ...
        for (index, job) in profile.jobs.enumerated() {
            ...
            case .ok(let value):
                guard let dict = value as? [String: Any],
                      let taskPayload = dict["task"] as? [String: Any] else {
                    return .err(code: "internal_error", message: "Profile job returned malformed payload", data: [
                        "profile": profile.name,
                        "group_id": groupId
                    ])
                }
    ...
    private func v2AgentTaskRunMany(params: [String: Any]) -> V2CallResult {
        ...
        for (index, job) in jobs.enumerated() {
            ...
            let label = v2AgentString(job["label"]) ?? v2AgentDefaultTaskLabel(for: command)
            var jobParams = params
            jobParams["session_id"] = sessionId
            jobParams["group_id"] = groupId
            jobParams["command"] = command
            jobParams["label"] = label

            if let splitRaw = v2AgentString(job["split"]) {
                jobParams["split"] = splitRaw
            }
            if let cwd = v2AgentString(job["cwd"]) {
                jobParams["cwd"] = cwd
            }
            if let surface = v2AgentString(job["surface_id"]) {
                jobParams["surface_id"] = surface
            }
            if let pane = v2AgentString(job["pane_id"]) {
                jobParams["pane_id"] = pane
            }
            if let workspace = v2AgentString(job["workspace_id"]) {
                jobParams["workspace_id"] = workspace
            }
            if let window = v2AgentString(job["window_id"]) {
                jobParams["window_id"] = window
            }
            if let profileName = v2AgentString(job["profile_name"]) {
                jobParams["profile_name"] = profileName
            }
            if let expectedPorts = v2StrictIntArrayAny(job["expected_ports"]) {
                jobParams["expected_ports"] = expectedPorts
            }

            if index > 0, jobParams["split"] == nil {
            ...
            case .ok(let value):
                guard let dict = value as? [String: Any],
                      let taskPayload = dict["task"] as? [String: Any] else {
                    return .err(code: "internal_error", message: "run_many job returned malformed payload", data: [
                        "session_id": sessionId,
                        "group_id": groupId,
                        "job_index": index
                    ])
    ...
    private func v2AgentJobsArray(from raw: Any?) -> [[String: Any]]? {
        if let jobs = raw as? [[String: Any]] {
            return jobs
        }

        guard let rawDict = raw as? [String: Any],
              let jobs = rawDict["jobs"] as? [[String: Any]] else {
            return nil
        }
        return jobs
    }
    ...
    private func v2AgentTaskLogs(params: [String: Any]) -> V2CallResult {
        ...
        if !["tail", "delta", "path-only"].contains(mode) {
            return .err(code: "invalid_params", message: "Unsupported logs mode", data: [
                "session_id": sessionId,
                "job_id": taskId,
                "mode": mode,
                "supported_modes": ["tail", "delta", "path-only"]
            ])
        }

        let requestedLines = v2Int(params, "lines")
        if let requestedLines, requestedLines <= 0 {
            return .err(code: "invalid_params", message: "lines must be greater than 0", data: nil)
        }
        let requestedBytes = v2Int(params, "bytes")
        if let requestedBytes, requestedBytes <= 0 {
            return .err(code: "invalid_params", message: "bytes must be greater than 0", data: nil)
        }
        let cursor = max(0, v2Int(params, "cursor") ?? 0)

        var result: V2CallResult = .err(code: "internal_error", message: "Failed to read task logs", data: nil)
        v2MainSync {
            guard let task = self.v2AgentTasks[taskId], task.sessionId == sessionId else {
                result = .err(code: "not_found", message: "Task not found", data: [
                    "session_id": sessionId,
                    "job_id": taskId
                ])
                return
            ...
            let (logPath, capturedText): (String?, String)
            if let taskTarget = self.v2AgentTaskTerminalTarget(task: task, session: session) {
                let terminalPanel = taskTarget.terminalPanel
                let loaded = self.v2AgentLoadTaskLog(task: task, terminalPanel: terminalPanel)
                logPath = loaded.path
                capturedText = loaded.text
            } else if let savedLogText {
                ...
                result = .ok([
                    ...
                    "mode": mode,
                    "status": task.status.rawValue,
                    "log_path": v2OrNull(savedLogPath),
                    "bytes": 0,
                    "cursor": 0,
            ...
            case "delta":
                let payloadText = self.v2AgentLimitedDeltaText(
                    fullText: fullText,
    ...
    private func v2AgentTaskCancel(params: [String: Any]) -> V2CallResult {
        ...
        v2MainSync {
            guard var task = self.v2AgentTasks[taskId], task.sessionId == sessionId else {
                result = .err(code: "not_found", message: "Task not found", data: [
                    "session_id": sessionId,
                    "job_id": taskId
                ])
            ...
            guard let target = self.v2AgentTaskTerminalTarget(task: task, session: session) else {
                result = .err(code: "invalid_state", message: "Task has no terminal target", data: [
                    "session_id": sessionId,
                    "job_id": taskId,
                    "status": task.status.rawValue
            ...
            let interruptDelivery = self.v2AgentSendInterrupt(terminalPanel: target.terminalPanel)
    ...
    private func v2AgentTaskTerminalTarget(
        task: AgentTask,
        session: AgentSession
    ) -> (context: AgentWorkspaceContext, terminalPanel: TerminalPanel)? {
        guard let workspaceId = task.workspaceId else { return nil }

        var contextParams: [String: Any] = [
            "session_id": session.id,
            "workspace_id": workspaceId.uuidString
        ]
        if let windowId = task.windowId {
            contextParams["window_id"] = windowId.uuidString
        }
        if let surfaceId = task.surfaceId {
            contextParams["surface_id"] = surfaceId.uuidString
        }
        if let paneId = task.paneId {
            contextParams["pane_id"] = paneId.uuidString
        }

        guard let resolvedContext = v2AgentResolveContext(params: contextParams, session: session) else {
            return nil
        }

        let selectedSurfaceId = task.surfaceId ?? resolvedContext.surfaceId
        guard let surfaceId = selectedSurfaceId,
              let terminalPanel = resolvedContext.workspace.terminalPanel(for: surfaceId) else {
            return nil
        }
        let paneId = resolvedContext.workspace.paneId(forPanelId: surfaceId)?.id
        return (
            context: AgentWorkspaceContext(
                windowId: resolvedContext.windowId,
                tabManager: resolvedContext.tabManager,
    ...
    private func v2AgentLoadTaskLog(
        task: AgentTask,
        terminalPanel: TerminalPanel
    ...
    private func v2AgentLimitedTailText(
        ...
        lineLimit: Int?,
        byteLimit: Int?
    ) -> (lines: [String], payloadBytes: Int) {
        let bounded = v2AgentBoundText(
            fullText,
            lineLimit: lineLimit ?? 80,
            byteLimit: byteLimit
        )
        let lines = v2AgentCompactTailLines(from: bounded, limit: lineLimit ?? 80)
        return (lines: lines, payloadBytes: bounded.utf8.count)
    }

    private func v2AgentLimitedDeltaText(
        fullText: String,
        fromCursor: Int,
        lineLimit: Int?,
        byteLimit: Int?
        ...
        let cursor = min(max(0, fromCursor), fullBytes.count)
        let deltaText = String(decoding: fullBytes[cursor...], as: UTF8.self)
        let bounded = v2AgentBoundText(
            deltaText,
            lineLimit: lineLimit,
            byteLimit: byteLimit
    ...
    private func v2AgentBoundText(_ text: String, lineLimit: Int?, byteLimit: Int?) -> String {
        var bounded = text
        if let byteLimit, byteLimit > 0 {
            let bytes = Array(bounded.utf8)
            if bytes.count > byteLimit {
                bounded = String(decoding: bytes.suffix(byteLimit), as: UTF8.self)
            }
        }
        if let lineLimit, lineLimit > 0 {
            bounded = v2AgentCompactTailLines(from: bounded, limit: lineLimit).joined(separator: "\n")
        }
        return bounded
    }

    private func v2AgentString(_ value: Any?) -> String? {
    ...
    private func v2AgentTaskWait(params: [String: Any]) -> V2CallResult {
        ...
        while true {
            var completedResult: V2CallResult?
            v2MainSync {
                if let task = self.v2AgentTasks[taskId], task.sessionId == sessionId, task.status.isTerminal {
    ...
    private func v2AgentReadEvents(params: [String: Any]) -> V2CallResult {
        ...
        let eventLogSnapshot = withV2AgentState {
            v2AgentEventLog
        }
        let events = eventLogSnapshot
            .filter { event in
    ...
    private func v2AgentServiceList(params: [String: Any]) -> V2CallResult {
        guard let sessionId = v2String(params, "session_id") else {
            return .err(code: "invalid_params", message: "Missing session_id", data: nil)
        }
        guard let session = withV2AgentState({ v2AgentSessions[sessionId] }) else {
    ...
    private func v2AgentStateSummary(params: [String: Any]) -> V2CallResult {
        ...
        let mainSnapshot = v2MainSync {
            (
                ...
                preferredBrowserSurface: v2AgentPreferredBrowserSurfaceRef(session: effectiveSession, context: context)
            )
        }
        let lastFailedJob = v2AgentLastFailedJobSummary(session: effectiveSession, context: context)

#if DEBUG
        dlog(
            "agent.state.summary cwd=\(workingDirectory ?? "nil") " +
            "activeDev=\((mainSnapshot.activeDevServer?["url"] as? String) ?? "nil") " +
            "failedJob=\((lastFailedJob?["job_id"] as? String) ?? "nil")"
        )
#endif
        return .ok([
            "session_id": sessionId,
            "layout_rev": effectiveSession.layoutRev,
            "active_dev_server": v2OrNull(mainSnapshot.activeDevServer),
            "preferred_browser_surface": v2OrNull(mainSnapshot.preferredBrowserSurface),
    ...
    private func v2AgentResolveContext(
        ...
        func buildContext(_ candidateParams: [String: Any]) -> AgentWorkspaceContext? {
            guard let tabManager = v2ResolveTabManager(params: candidateParams) else {
                return nil
            }
            return v2MainSync {
                guard let workspace = v2ResolveWorkspace(params: candidateParams, tabManager: tabManager) else {
                    return nil
                }
    ...
    private func v2AgentWorkspaceDirectory(context: AgentWorkspaceContext) -> String? {
        v2MainSync {
            v2AgentString(context.workspace.currentDirectory)
        }
    }
    ...
    private func v2AgentLatestTask(
        sessionId: String,
        workspaceId: UUID,
        surfaceId: UUID?
    ) -> AgentTask? {
        let matchingTasks = withV2AgentState {
            Array(v2AgentTasks.values)
        }.filter { task in
            guard task.sessionId == sessionId, task.workspaceId == workspaceId else {
                return false
            }
            if let surfaceId {
                return task.surfaceId == surfaceId
            }
            return true
        }
        return matchingTasks.sorted { lhs, rhs in
    ...
    private func v2AgentActiveDevServer(
        ...
        if let preferredService = services.sorted(by: { lhs, rhs in
            ...
            let lhsTask = (lhs["job_id"] as? String).flatMap { tasksById[$0] }
            let rhsTask = (rhs["job_id"] as? String).flatMap { tasksById[$0] }
    ...
    private func v2AgentWorkspaceFingerprint(
        ...
        let availableTools = repoRoot == nil ? [] : v2AgentAvailableTools()
        let profiles = v2AgentAvailableProfiles(
            repoRoot: repoRoot,
            availableTools: availableTools
        )
        let workspaceSnapshot = v2MainSync {
            (
                listeningPorts: context.workspace.listeningPorts.sorted(),
                surfaceCount: context.workspace.panels.count
            )
        }
        let fingerprintPayload: [String: Any] = [
            ...
            "profiles": profiles,
            "listening_ports": workspaceSnapshot.listeningPorts,
            "surface_count": workspaceSnapshot.surfaceCount
        ]
    ...
    private func v2AgentArtifactPayloads(
        ...
        let taskSnapshot = withV2AgentState {
            Array(v2AgentTasks.values)
        }
        let candidateTasks = taskSnapshot
            .filter { task in
        ...
        let recordedArtifactSnapshot = withV2AgentState {
            v2AgentRecordedArtifacts
        }
        let recordedArtifacts = recordedArtifactSnapshot
            .filter { artifact in
                let matchesScope =
    ...
    private func v2AgentResolveTerminalTarget(
        ...
        guard let context = v2AgentResolveContext(params: params, session: session),
              let surfaceId = context.surfaceId,
              let terminalPanel = context.workspace.terminalPanel(for: surfaceId) else {
            return nil
        }
        let paneId = context.workspace.paneId(forPanelId: surfaceId)?.id ?? context.paneId
        let terminalContext = AgentWorkspaceContext(
            windowId: context.windowId,
            tabManager: context.tabManager,
            workspace: context.workspace,
        ...
        return (terminalContext, terminalPanel)
    }

    private func v2AgentSendText(_ text: String, terminalPanel: TerminalPanel) -> Bool {
        if let surface = terminalPanel.surface.surface {
            sendSocketText(text, surface: surface)
            terminalPanel.surface.forceRefresh(reason: "terminalController.agentTerminalWrite")
            return false
        }
    ...
    private func v2AgentSendInterrupt(terminalPanel: TerminalPanel) -> String {
        if let surface = terminalPanel.surface.surface {
            _ = sendNamedKey(surface, keyName: "ctrl-c")
            return "live_surface"
        }
    ...
    private func v2AgentTaskPayload(_ task: AgentTask) -> [String: Any] {
        var payload: [String: Any] = [
            ...
            "workspace_ref": v2Ref(kind: .workspace, uuid: task.workspaceId),
            "pane_id": v2OrNull(task.paneId?.uuidString),
            "pane_ref": v2Ref(kind: .pane, uuid: task.paneId),
            "surface_id": v2OrNull(task.surfaceId?.uuidString),
            "surface_ref": v2Ref(kind: .surface, uuid: task.surfaceId),
            "created_at": v2AgentISO8601String(task.createdAt),
            "updated_at": v2AgentISO8601String(task.updatedAt),
            "started_at": v2OrNull(v2AgentISO8601String(task.startedAt)),
            "completed_at": v2OrNull(v2AgentISO8601String(task.completedAt)),
    ...
    private func v2AgentISO8601String(_ date: Date?) -> String? {
        ...
        return v2AgentISO8601String(date)
    }

    private func v2AgentTaskDurationMs(_ task: AgentTask) -> Int? {
        let endDate = task.completedAt ?? task.updatedAt
        let startDate = task.startedAt ?? task.createdAt
    ...
    private func v2AgentReadFileTail(path: String, maxBytes: Int) -> String? {
        guard maxBytes > 0,
    ...
    private func v2AgentPackageScripts(repoRoot: String?) -> [String: String] {
        guard let repoRoot else { return [:] }
        let packageJSONURL = URL(fileURLWithPath: repoRoot, isDirectory: true).appendingPathComponent("package.json")
    ...
    private func v2AgentHasFile(repoRoot: String?, named fileName: String) -> Bool {
        guard let repoRoot else { return false }
        let fileURL = URL(fileURLWithPath: repoRoot, isDirectory: true).appendingPathComponent(fileName)
        return FileManager.default.fileExists(atPath: fileURL.path)
    }

    private func v2AgentScriptCommand(scriptName: String, packageManager: String?) -> String {
    ...
    private func v2AgentTSCCommand(packageManager: String?, availableTools: [String]) -> String? {
        if packageManager == "bun" && availableTools.contains("bun") {
            return "bun x tsc --noEmit"
        }
        if availableTools.contains("tsc") {
            return "tsc --noEmit"
        }
        return nil
    }

    private func v2AgentProfilePortHints(from script: String) -> [Int] {
        let matches = v2AgentRegexCaptureGroups(
            pattern: #"--port(?:=|\s+)(\d{2,5})"#,
            text: script
        )
        let ports: [Int] = matches.compactMap { groups -> Int? in
    ...
    private func v2AgentPanePanelIds(
        ...
        guard let paneUUID,
              let paneId = v2AgentPaneId(for: paneUUID, workspace: workspace) else {
            return []
        }

        var panelIds: [UUID] = []
        if let selectedPanelId = workspace.effectiveSelectedPanelId(inPane: paneId) {
            panelIds.append(selectedPanelId)
        }
        for tab in workspace.bonsplitController.tabs(inPane: paneId) {
            guard let panelId = workspace.panelIdFromSurfaceId(tab.id),
                  !panelIds.contains(panelId) else { continue }
    ...
    private func v2AgentBrowserMatchesURL(
        ...
        guard let requestedURL, !requestedURL.isEmpty else { return true }
        return browserPanel.currentURL?.absoluteString == requestedURL
    }

    private func v2AgentReusableBrowserSurfaceId(
        params: [String: Any],
        context: AgentWorkspaceContext,
        requestedURL: String?
    ) -> UUID? {
        ...
        let workspace = context.workspace
        if let explicitSurfaceId = v2UUID(params, "surface_id"),
           let browserPanel = workspace.browserPanel(for: explicitSurfaceId),
           v2AgentBrowserMatchesURL(browserPanel, requestedURL: requestedURL) {
            return explicitSurfaceId
        }

        let candidatePaneUUID = v2UUID(params, "pane_id") ?? context.paneId
        return v2AgentPanePanelIds(paneUUID: candidatePaneUUID, workspace: workspace)
            .first(where: { panelId in
                guard let browserPanel = workspace.browserPanel(for: panelId) else { return false }
                return v2AgentBrowserMatchesURL(browserPanel, requestedURL: requestedURL)
            })
    }

    private func v2AgentResolveProfile(
        name: String,
        context: AgentWorkspaceContext
    ) -> AgentProfileSpec? {
        ...
        case "verify.ts":
            ...
            if scripts["typecheck"] != nil {
                jobs.append(.init(label: "typecheck", command: v2AgentScriptCommand(scriptName: "typecheck", packageManager: preferredPackageManager), split: "right"))
            } else if scripts["check-types"] != nil {
                jobs.append(.init(label: "check-types", command: v2AgentScriptCommand(scriptName: "check-types", packageManager: preferredPackageManager), split: "right"))
            } else if v2AgentHasFile(repoRoot: repoRoot, named: "tsconfig.json"),
                      let tscCommand = v2AgentTSCCommand(packageManager: preferredPackageManager, availableTools: availableTools) {
        ...
        case "verify.flutter":
            guard availableTools.contains("flutter"),
                  v2AgentHasFile(repoRoot: repoRoot, named: "pubspec.yaml") else {
                return nil
            }
            return AgentProfileSpec(
                name: name,
    ...
    private func v2AgentTask(
        _ task: AgentTask,
        matchesServicePort port: Int,
        in workspace: Workspace
    ) -> Bool {
        if task.expectedPorts.contains(port) {
            return true
        }
        guard let surfaceId = task.surfaceId else { return false }
        return workspace.surfaceListeningPorts[surfaceId]?.contains(port) == true
    }
    ...
    private func v2AgentEnvironmentPayload(
        ...
        let cwd = v2AgentWorkspaceDirectory(context: context)
        ...
        let searchState = repoRoot.flatMap { repoRoot in
            withV2AgentState { v2AgentSearchIndexByRepo[repoRoot] }
        }
    ...
    private func v2AgentResolvedCommandPath(_ executable: String) -> String? {
        ...
        let allowedPrefixes = [homeDirectory, "/opt", "/usr", "/bin", "/sbin"]
        var searchDirectories: [String] = []
        var seenDirectories = Set<String>()

        func appendSearchPath(_ path: String?) {
            guard let path else { return }
            for rawComponent in path.split(separator: ":").prefix(24) {
                let component = String(rawComponent).trimmingCharacters(in: .whitespacesAndNewlines)
                guard !component.isEmpty,
                      component.hasPrefix("/"),
                      allowedPrefixes.contains(where: { component.hasPrefix($0) }),
        ...
        let cwd = v2String(params, "cwd")
            ...
            ?? session.flatMap { session in
                v2AgentResolveContext(params: params, session: session).flatMap { context in
                    v2AgentWorkspaceDirectory(context: context)
                }
            }
    ...
    private func v2StrictIntArray(_ params: [String: Any], _ key: String) -> [Int]? {
        v2StrictIntArrayAny(params[key])
    }

    private func v2StrictIntArrayAny(_ raw: Any?) -> [Int]? {
        guard let raw else { return nil }
        if let values = raw as? [Int] {
            return values
        }
        if let values = raw as? [NSNumber] {
            let resolved = values.compactMap { v2StrictIntAny($0) }
            return resolved.count == values.count ? resolved : nil
        }
        if let values = raw as? [Any] {
            let resolved = values.compactMap { v2StrictIntAny($0) }
            return resolved.count == values.count ? resolved : nil
        }
        if let single = v2StrictIntAny(raw) {
            return [single]
        }
        return nil
    }
    ...
    private func helpText() -> String {
        ...
        Notification commands:
          ...
          report_task_result <task_id> <exit_code> [--tab=X] [--panel=Y] - Report completion of an agent-managed task
    ...
    private func reportTaskResult(_ args: String) -> String {
        let parsed = parseOptions(args)
        guard let taskId = parsed.positional.first, !taskId.isEmpty else {
            return "ERROR: Missing task id — usage: report_task_result <task_id> <exit_code> [--tab=X] [--panel=Y]"
        }
        guard parsed.positional.count >= 2,
              let exitCode = Int(parsed.positional[1]) else {
            return "ERROR: Missing or invalid exit code — usage: report_task_result <task_id> <exit_code> [--tab=X] [--panel=Y]"
        }

        let workspaceId = parsed.options["tab"].flatMap(UUID.init(uuidString:))
        let surfaceId = (parsed.options["panel"] ?? parsed.options["surface"]).flatMap(UUID.init(uuidString:))
        ...
        DispatchQueue.main.async {
            ...
            if let surfaceId, task.surfaceId == nil {
                task.surfaceId = surfaceId
            }
