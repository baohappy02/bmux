...
private func bmuxIsLikelyWebInspectorResponder(_ responder: NSResponder?) -> Bool {
    guard let responder else { return false }
...
/// For browser content, let the page try the Find command family before bmux's menu fallback.
/// This preserves native web-app shortcuts like VS Code's Cmd+F while still allowing bmux's
...
func bmuxOwningGhosttyView(for responder: NSResponder?) -> GhosttyNSView? {
    ...
    if let textView = responder as? NSTextView,
       ...
       let ghosttyView = bmuxOwningGhosttyView(for: delegateView) {
        return ghosttyView
    }
...
final class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate, NSMenuItemValidation {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if telemetryEnabled && !sentryDSN.isEmpty {
    ...
    private func handleCustomShortcut(event: NSEvent) -> Bool {
        ...
        if browserAddressBarFocusedPanelId != nil,
           bmuxOwningGhosttyView(for: NSApp.keyWindow?.firstResponder) != nil {
#if DEBUG
        ...
        if !normalizedFlags.contains(.command),
           let ghosttyView = bmuxOwningGhosttyView(for: NSApp.keyWindow?.firstResponder),
        ...
        if matchShortcut(
            ...
            if let targetWindow = [NSApp.keyWindow, event.window]
                .compactMap({ $0 })
                .first(where: { $0.identifier?.rawValue == "bmux.browser-popup" }) {
#if DEBUG
               ...
               bmuxWindowShouldOwnCloseShortcut(targetWindow) {
                targetWindow.performClose(nil)
            } else {
                let targetWindow = event.window ?? NSApp.keyWindow ?? NSApp.mainWindow
                if let terminalContext = focusedTerminalShortcutContext(preferredWindow: targetWindow) {
    ...
    private func isLikelyWebInspectorResponder(_ responder: NSResponder?) -> Bool {
        bmuxIsLikelyWebInspectorResponder(responder)
    }
    ...
    fileprivate func browserFindBarIsVisible(for webView: BmuxWebView) -> Bool {
...
private var bmuxBrowserReturnForwardingDepth = 0
private var bmuxWindowFirstResponderBypassDepth = 0
private var bmuxFieldEditorOwningWebViewAssociationKey: UInt8 = 0

@discardableResult
func bmuxWithWindowFirstResponderBypass<T>(_ body: () -> T) -> T {
    bmuxWindowFirstResponderBypassDepth += 1
    defer {
        bmuxWindowFirstResponderBypassDepth = max(0, bmuxWindowFirstResponderBypassDepth - 1)
    }
    return body()
}

func bmuxIsWindowFirstResponderBypassActive() -> Bool {
    bmuxWindowFirstResponderBypassDepth > 0
}
...
private extension AppDelegate {
    @objc func handleThemesReloadNotification(_ notification: Notification) {
        DispatchQueue.main.async {
            GhosttyApp.shared.reloadConfiguration(source: "distributed.bmux.themes")
        }
    }
}
...
private extension NSWindow {
    @objc func bmux_makeFirstResponder(_ responder: NSResponder?) -> Bool {
        if bmuxIsWindowFirstResponderBypassActive() {
#if DEBUG
            dlog(
    ...
    @objc func bmux_sendEvent(_ event: NSEvent) {
        ...
        let typingTimingExtra: String? = {
            ...
            let hitWebView = Self.bmuxHitViewForEventDispatch(in: self, event: event).flatMap {
                Self.bmuxOwningWebView(for: $0)
            }
        ...
        bmuxFirstResponderGuardHitViewContext = Self.bmuxHitViewForEventDispatch(in: self, event: event)
    ...
    @objc func bmux_performKeyEquivalent(with event: NSEvent) -> Bool {
        ...
        if shouldDispatchBrowserReturnViaFirstResponderKeyDown(
            ...
            if bmuxBrowserReturnForwardingDepth > 0 {
#if DEBUG
                dlog("  → browser Return/Enter reentry; using normal dispatch")
#endif
                return false
            }
            bmuxBrowserReturnForwardingDepth += 1
            defer { bmuxBrowserReturnForwardingDepth = max(0, bmuxBrowserReturnForwardingDepth - 1) }
#if DEBUG
    ...
    private static func bmuxIsPointerDownEvent(_ event: NSEvent) -> Bool {
        switch event.type {
    ...
    private static func bmuxPointerHitWebView(in window: NSWindow, event: NSEvent) -> BmuxWebView? {
        guard bmuxIsPointerDownEvent(event) else { return nil }
        if event.windowNumber != 0, event.windowNumber != window.windowNumber {
            return nil
        }
        if let eventWindow = event.window, eventWindow !== window {
