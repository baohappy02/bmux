...
struct BmuxSurfaceConfigTemplate {
...
private func bmuxPointerAppearsLive(_ pointer: UnsafeMutableRawPointer?) -> Bool {
    guard let pointer,
...
func bmuxSurfacePointerAppearsLive(_ surface: ghostty_surface_t) -> Bool {
    ...
    bmuxPointerAppearsLive(surface)
}

func bmuxCurrentSurfaceFontSizePoints(_ surface: ghostty_surface_t) -> Float? {
    guard bmuxSurfacePointerAppearsLive(surface) else {
        return nil
    }
...
func bmuxDefaultWorkingDirectoryPath() -> String {
    let fileManager = FileManager.default
    return fileManager.urls(for: .desktopDirectory, in: .userDomainMask).first?.path
...
func bmuxInheritedSurfaceConfig(
    ...
    var config = BmuxSurfaceConfigTemplate(cConfig: inherited)
...
extension Workspace {
    ...
    func applyCustomLayout(_ layout: BmuxLayoutNode, baseCwd: String) {
...
private final class WorkspaceRemoteDaemonRPCClient {
    ...
    private let writeQueue = DispatchQueue(label: "com.bmux.remote-ssh.daemon-rpc.write.\(UUID().uuidString)")
    private let stateQueue = DispatchQueue(label: "com.bmux.remote-ssh.daemon-rpc.state.\(UUID().uuidString)")
...
private final class WorkspaceRemoteDaemonProxyTunnel {
    private final class ProxySession {
        ...
        private static let remoteLoopbackProxyAliasHost = "bmux-loopback.localtest.me"
...
final class WorkspaceRemoteSessionController {
    ...
    private let queue = DispatchQueue(label: "com.bmux.remote-ssh.\(UUID().uuidString)", qos: .utility)
    ...
    static func remoteRelayMetadataCleanupScript(relayPort: Int) -> String {
        ...
        rm -f "$HOME/.bmux/relay/\(relayPort).auth" "$HOME/.bmux/relay/\(relayPort).daemon_path"
        """
    }
    ...
    private func probeRemoteBootstrapStateLocked(version: String) throws -> RemoteBootstrapState {
        ...
        case "$(printf '%s' "$bmux_uname_os" | tr '[:upper:]' '[:lower:]')" in
          linux|darwin|freebsd) bmux_go_os="$(printf '%s' "$bmux_uname_os" | tr '[:upper:]' '[:lower:]')" ;;
          *) exit 70 ;;
        esac
        case "$(printf '%s' "$bmux_uname_arch" | tr '[:upper:]' '[:lower:]')" in
          x86_64|amd64) bmux_go_arch=amd64 ;;
          aarch64|arm64) bmux_go_arch=arm64 ;;
          armv7l) bmux_go_arch=arm ;;
          *) exit 71 ;;
        esac
        bmux_remote_path="$HOME/.bmux/bin/bmuxd-remote/\(version)/${bmux_go_os}-${bmux_go_arch}/bmuxd-remote"
    ...
    private func buildLocalDaemonBinary(goOS: String, goArch: String, version: String) throws -> URL {
        ...
        guard Self.allowLocalDaemonBuildFallback() else {
            throw NSError(domain: "bmux.remote.daemon", code: 20, userInfo: [
                NSLocalizedDescriptionKey: "this build does not include a verified bmuxd-remote manifest for \(goOS)-\(goArch). Use a release/nightly build, or set CMUX_REMOTE_DAEMON_ALLOW_LOCAL_BUILD=1 for a dev-only fallback.",
            ])
        }
    ...
    static func remoteCLIWrapperInstallScript(daemonRemotePath: String) -> String {
        ...
        ln -sf "$HOME/\(trimmedRemotePath)" "$HOME/.bmux/bin/bmuxd-remote-current"
    ...
    static func remoteRelayMetadataInstallScript(
        ...
        printf '%s' "$HOME/\(trimmedRemotePath)" > "$HOME/.bmux/relay/\(relayPort).daemon_path"
        cat > "$HOME/.bmux/relay/\(relayPort).auth" <<'CMUXRELAYAUTH'
        \(authPayload)
        CMUXRELAYAUTH
...
final class Workspace: Identifiable, ObservableObject {
    ...
    private func sidebarHomeDirectoryForCanonicalization(
        ...
        return bmuxDefaultWorkingDirectoryPath()
    }
