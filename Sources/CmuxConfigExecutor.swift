...
struct BmuxConfigExecutor {
    ...
    static func execute(
        ...
        if let workspace = command.workspace {
            ...
            if needsConfirm, let sourcePath = configSourcePath {
                let trusted = BmuxDirectoryTrust.shared.isTrusted(
    ...
    private static func showConfirmDialog(command: String, configPath: String) -> Bool {
        ...
        let checkbox = NSButton(checkboxWithTitle: String(
            localized: "dialog.bmuxConfig.confirmCommand.trustDirectory",
        ...
        if checkbox.state == .on {
            BmuxDirectoryTrust.shared.trust(configPath: configPath)
        }
    ...
    private static func executeWorkspaceCommand(
        ...
        if let existing = tabManager.tabs.first(where: { $0.customTitle == workspaceName }) {
            ...
            case .confirm:
                ...
                alert.alertStyle = .warning
                alert.addButton(withTitle: String(localized: "dialog.bmuxConfig.confirmRestart.recreate", defaultValue: "Recreate"))
                alert.addButton(withTitle: String(localized: "dialog.bmuxConfig.confirmRestart.cancel", defaultValue: "Cancel"))
