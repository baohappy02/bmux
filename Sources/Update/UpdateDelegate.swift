...
enum UpdateFeedResolver {
    ...
    static func resolvedFeedURLString(infoFeedURL: String?) -> (url: String?, isNightly: Bool, usedFallback: Bool) {
        guard let infoFeedURL, !infoFeedURL.isEmpty else {
            return (fallbackFeedURL.isEmpty ? nil : fallbackFeedURL, false, true)
        }
...
extension UpdateDriver: SPUUpdaterDelegate {
    ...
    func feedURLString(for updater: SPUUpdater) -> String? {
        ...
        guard let url = resolved.url, !url.isEmpty else {
            UpdateLogStore.shared.append("update channel: disabled")
            recordFeedURLString("<disabled>", usedFallback: resolved.usedFallback)
            return nil
        }
        UpdateLogStore.shared.append("update channel: \(resolved.isNightly ? "nightly" : "stable")")
        recordFeedURLString(url, usedFallback: resolved.usedFallback)
        return url
    }
