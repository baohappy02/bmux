...
#if os(macOS)
func bmuxShouldApplyWindowGlass(
...
func bmuxShouldUseTransparentBackgroundWindow() -> Bool {
    ...
    return bmuxShouldApplyWindowGlass(
...
func bmuxShouldUseClearWindowBackground(for opacity: Double) -> Bool {
    bmuxShouldUseTransparentBackgroundWindow() || opacity < 0.999
}

private func bmuxTransparentWindowBaseColor() -> NSColor {
...
#if DEBUG
private func bmuxChildExitProbePath() -> String? {
...
private func bmuxLoadChildExitProbe(at path: String) -> [String: String] {
...
private func bmuxWriteChildExitProbe(_ updates: [String: String], increments: [String: Int] = [:]) {
    guard let path = bmuxChildExitProbePath() else { return }
    var payload = bmuxLoadChildExitProbe(at: path)
...
private func bmuxScalarHex(_ value: String?) -> String {
...
#if DEBUG
func bmuxPasteboardStringContentsForTesting(_ pasteboard: NSPasteboard) -> String? {
    GhosttyPasteboardHelper.stringContents(from: pasteboard)
}

func bmuxPasteboardImageFileURLForTesting(_ pasteboard: NSPasteboard) -> URL? {
    GhosttyPasteboardHelper.saveImageFileURLIfNeeded(from: pasteboard)
}

func bmuxPasteboardImagePathForTesting(_ pasteboard: NSPasteboard) -> String? {
...
class GhosttyApp {
    ...
    let backgroundLogEnabled = {
        ...
        if UserDefaults.standard.bool(forKey: "bmuxDebugBG") {
            return true
        }
    ...
    private func loadDefaultConfigFilesWithLegacyFallback(_ config: ghostty_config_t) {
        ...
        // bmux provides the terminal background via backgroundView (CALayer)
    ...
    /// font already covers the ranges bmux would otherwise auto-map.
    ...
    /// application-support config paths that bmux may load at runtime.
    ...
    /// Returns the top-level config paths that bmux will actually load before
    ...
    /// Scans a single config file for font settings relevant to bmux's
    ...
    private func handleAction(target: ghostty_target_s, action: ghostty_action_s) -> Bool {
        ...
        case GHOSTTY_ACTION_OPEN_URL:
            ...
            if !BrowserLinkOpenSettings.openTerminalLinksInBmuxBrowser() {
                #if DEBUG
                dlog("link.openURL bmuxBrowser=disabled, opening externally url=\(target.url)")
                #endif
                return performOnMain {
    ...
    private func applyBackgroundToKeyWindow() {
        ...
        if bmuxShouldUseClearWindowBackground(for: defaultBackgroundOpacity) {
            window.backgroundColor = bmuxTransparentWindowBaseColor()
    ...
    func logBackground(_ message: String) {
        ...
        let line =
            "\(timestamp) seq=\(sequence) t+\(String(format: "%.3f", uptimeMs))ms thread=\(threadLabel) frame60=\(frame60) frame120=\(frame120) bmux bg: \(message)\n"
...
final class TerminalSurface: Identifiable, ObservableObject {
    ...
    private static let sessionPortBase: Int = {
        let val = UserDefaults.standard.integer(forKey: "bmuxPortBase")
        return val > 0 ? val : 9100
    }()
    private static let sessionPortRangeSize: Int = {
        let val = UserDefaults.standard.integer(forKey: "bmuxPortRange")
        return val > 0 ? val : 10
    }()
    private let surfaceContext: ghostty_surface_context_e
    private let configTemplate: BmuxSurfaceConfigTemplate?
    ...
    func debugSurfaceContextLabel() -> String {
        bmuxSurfaceContextName(surfaceContext)
    }
    ...
    func liveSurfaceForGhosttyAccess(reason: String) -> ghostty_surface_t? {
        ...
        guard registeredOwnerId == id,
              bmuxSurfacePointerAppearsLive(surface) else {
...
#if DEBUG
    private static let surfaceLogPath = "/tmp/bmux-ghostty-surface.log"
    private static let sizeLogPath = "/tmp/bmux-ghostty-size.log"
    ...
    private func createSurface(for view: GhosttyNSView) {
        ...
        let baseConfig = configTemplate ?? BmuxSurfaceConfigTemplate()
        ...
        dlog(
            "zoom.create surface=\(id.uuidString.prefix(5)) context=\(bmuxSurfaceContextName(surfaceContext)) " +
        ...
        if shellIntegrationEnabled,
           let integrationDir = Bundle.main.resourceURL?.appendingPathComponent("shell-integration").path {
            ...
            if shellName == "zsh" {
                ...
                // not reliable. Bootstrap bmux bash integration on the first
                ...
                _bmux_bash_integration="$CMUX_SHELL_INTEGRATION_DIR/bmux-bash-integration.bash"; \
        ...
        if let inheritedFontPoints = configTemplate?.fontSize,
           inheritedFontPoints > 0 {
            let currentFontPoints = bmuxCurrentSurfaceFontSizePoints(createdSurface)
            let shouldReapply = {
...
#if DEBUG
        let runtimeFontText = bmuxCurrentSurfaceFontSizePoints(createdSurface).map {
            String(format: "%.2f", $0)
        } ?? "nil"
        dlog(
            "zoom.create.done surface=\(id.uuidString.prefix(5)) context=\(bmuxSurfaceContextName(surfaceContext)) " +
...
class GhosttyNSView: NSView, NSUserInterfaceValidations {
    ...
    func applyWindowBackgroundIfActive() {
        ...
        let color = effectiveBackgroundColor()
        if bmuxShouldUseClearWindowBackground(for: color.alphaComponent) {
            window.backgroundColor = bmuxTransparentWindowBaseColor()
            window.isOpaque = false
        ...
        if GhosttyApp.shared.backgroundLogEnabled {
            let signature = "\(bmuxShouldUseClearWindowBackground(for: color.alphaComponent) ? "transparent" : color.hexString()):\(String(format: "%.3f", color.alphaComponent))"
            if signature != lastLoggedWindowBackgroundSignature {
                ...
                let source = hasOverride ? "surfaceOverride" : "defaultBackground"
                GhosttyApp.shared.logBackground(
                    "window background applied tab=\(tabId?.uuidString ?? "unknown") surface=\(terminalSurface?.id.uuidString ?? "unknown") source=\(source) override=\(overrideHex) default=\(defaultHex) transparent=\(bmuxShouldUseClearWindowBackground(for: color.alphaComponent)) color=\(color.hexString()) opacity=\(String(format: "%.3f", color.alphaComponent))"
                )
            }
        }
    }
    ...
    override func performKeyEquivalent(with event: NSEvent) -> Bool {
        ...
        bmuxWriteChildExitProbe(
            [
                "probePerformCharsHex": bmuxScalarHex(event.characters),
                "probePerformCharsIgnoringHex": bmuxScalarHex(event.charactersIgnoringModifiers),
    ...
    override func keyDown(with event: NSEvent) {
        ...
        bmuxWriteChildExitProbe(
            [
                "probeKeyDownCharsHex": bmuxScalarHex(event.characters),
                "probeKeyDownCharsIgnoringHex": bmuxScalarHex(event.charactersIgnoringModifiers),
        ...
        if flags.contains(.control) && !flags.contains(.command) && !flags.contains(.option) && !hasMarkedText() {
            ...
            dlog(
                ...
                "handled=\(handled ? 1 : 0) keyCode=\(event.keyCode) chars=\(bmuxScalarHex(event.characters)) " +
                "ign=\(bmuxScalarHex(event.charactersIgnoringModifiers)) mods=\(event.modifierFlags.rawValue)"
            )
#endif
...
extension GhosttyNSView: NSTextInputClient {
    ...
    fileprivate func sendTextToSurface(_ chars: String, preserveLiteralEscape: Bool) {
        ...
        bmuxWriteChildExitProbe(
            [
                "probeInsertTextCharsHex": bmuxScalarHex(chars),
