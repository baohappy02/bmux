...
/// Manages trusted directories for bmux.json command execution.
/// When a directory (or its git repo root) is trusted, `confirm: true` commands
/// from that directory's bmux.json skip the confirmation dialog.
/// Global config (~/.config/bmux/bmux.json) is always trusted.
final class BmuxDirectoryTrust {
    static let shared = BmuxDirectoryTrust()
    ...
    /// Check if a bmux.json path is trusted.
    /// Global config is always trusted. For local configs, check the git repo root
    /// (or the bmux.json parent directory if not in a git repo).
    ...
    /// Trust the directory containing a bmux.json. If the bmux.json is inside a git
    ...
    /// Resolve the trust key for a bmux.json path: git repo root if inside a repo,
    /// otherwise the bmux.json's parent directory.
