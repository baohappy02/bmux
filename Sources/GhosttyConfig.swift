...
struct GhosttyConfig {
    ...
    private static let bmuxReleaseBundleIdentifier = "com.bmuxterm.app"
    ...
    private static func bmuxConfigPaths(
        ...
        let releasePaths = paths(for: bmuxReleaseBundleIdentifier)
        guard let currentBundleIdentifier, !currentBundleIdentifier.isEmpty else {
            return releasePaths
        }
        if currentBundleIdentifier == bmuxReleaseBundleIdentifier {
            return releasePaths
        }
    ...
    private static func loadFromDisk(preferredColorScheme: ColorSchemePreference) -> GhosttyConfig {
        ...
        ].map { NSString(string: $0).expandingTildeInPath } + bmuxConfigPaths()
