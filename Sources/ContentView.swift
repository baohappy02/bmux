...
let tmuxWorkspacePaneOverlayContainerIdentifier = NSUserInterfaceItemIdentifier("bmux.tmuxWorkspacePane.overlay.container")
...
struct ContentView: View {
    ...
    var body: some View {
        ...
        view = AnyView(view.background(WindowAccessor { [sidebarBlendMode, bgGlassEnabled, bgGlassTintHex, bgGlassTintOpacity] window in
            ...
            let shouldApplyWindowGlass = bmuxShouldApplyWindowGlass(
    ...
    private func commandPaletteCommandContributions() -> [CommandPaletteCommandContribution] {
        ...
        let bmuxConfigDefaultSubtitle = constant(String(localized: "command.bmuxConfig.subtitle", defaultValue: "bmux.json"))
        for command in bmuxConfigStore.loadedCommands {
            let commandName = sanitizeBmuxConfigPaletteText(command.name)
            ...
            contributions.append(
                CommandPaletteCommandContribution(
                    ...
                    title: constant(String(localized: "command.bmuxConfig.customTitle", defaultValue: "Custom: \(commandName)")),
                    subtitle: subtitle,
    ...
    private func registerCommandPaletteHandlers(_ registry: inout CommandPaletteHandlerRegistry) {
        ...
        for command in bmuxConfigStore.loadedCommands {
            ...
            let sourcePath = bmuxConfigStore.commandSourcePaths[command.id]
            let globalPath = bmuxConfigStore.globalConfigPath
...
private enum FeedbackComposerSettings {
    ...
    static func endpointURL() -> URL? {
        ...
        guard !defaultEndpoint.isEmpty else { return nil }
...
enum SidebarDragLifecycleNotification {
    static let stateDidChange = Notification.Name("bmux.sidebarDragStateDidChange")
    static let requestClear = Notification.Name("bmux.sidebarDragRequestClear")
...
private struct TabItemView: View, Equatable {
    ...
    private var activeUnreadBadgeFillColor: Color {
        ...
        return usesInvertedActiveForeground ? Color.white.opacity(0.25) : bmuxAccentColor()
    }
    ...
    private var activeProgressFillColor: Color {
        usesInvertedActiveForeground ? Color.white.opacity(0.8) : bmuxAccentColor()
    }
    ...
    private var backgroundColor: Color {
        ...
        case .leftRail:
            ...
            if isMultiSelected { return bmuxAccentColor().opacity(0.25) }
            return Color.clear
        case .solidFill:
            if isActive { return Color(nsColor: selectionBackgroundColor) }
...
private enum SidebarTabDragPayload {
    ...
    private static let prefix = "bmux.sidebar-tab."
