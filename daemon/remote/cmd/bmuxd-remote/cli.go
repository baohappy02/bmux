...
doneFlags:
	...
	if socketPath == "" {
		fmt.Fprintln(os.Stderr, "bmux: CMUX_SOCKET_PATH not set and --socket not provided")
		return 1
	}
	...
	default:
		fmt.Fprintf(os.Stderr, "bmux: internal error: unknown protocol for %q\n", cmdName)
		return 1
	}
}
...
func runBrowserRelay(socketPath string, args []string, jsonOutput bool, refreshAddr func() string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "bmux browser: requires a subcommand (open, navigate, back, forward, reload, get-url)")
		return 2
	}
...
// when CMUX_SOCKET_PATH is not set. Written by the bmux app after the relay establishes.
