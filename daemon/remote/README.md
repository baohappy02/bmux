...
3. `bmuxd-remote cli <command> [args...]` — relay bmux commands to the local app over the reverse SSH forward
...
The `cli` subcommand (or `bmux` wrapper/symlink) connects to the local bmux app through an SSH reverse forward and relays commands. It supports both v1 text protocol and v2 JSON-RPC commands.
