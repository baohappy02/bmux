...
XCODE_LOG="/tmp/bmux-xcodebuild-${TAG_SLUG}.log"
set +e
...
if [[ -n "$TAG" && "$APP_NAME" != "$SEARCH_APP_NAME" ]]; then
  ...
  if [[ -f "$INFO_PLIST" ]]; then
    ...
    if [[ -n "${TAG_SLUG:-}" ]]; then
      ...
      CMUX_SOCKET="/tmp/bmux-debug-${TAG_SLUG}.sock"
...
if [[ -x "$CLI_PATH" ]]; then
  ...
  CMUX_SHIM_TARGET="$(select_bmux_shim_target || true)"
  if [[ -n "${CMUX_SHIM_TARGET:-}" ]]; then
    write_dev_cli_shim "$CMUX_SHIM_TARGET" "/Applications/bmux.app/Contents/Resources/bin/bmux"
  fi
fi

# Build bmuxd and ghostty helper binaries (needed for both launch and no-launch).
