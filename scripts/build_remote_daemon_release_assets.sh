...
Builds bmuxd-remote release assets for the supported remote platforms and emits:
  bmuxd-remote-<goos>-<goarch>[-<suffix>]
  bmuxd-remote-checksums[-<suffix>].txt
  bmuxd-remote-manifest[-<suffix>].json
...
if ! command -v go >/dev/null 2>&1; then
  echo "error: go is required to build bmuxd-remote release assets" >&2
  exit 1
fi
...
rm -f "$OUTPUT_DIR"/bmuxd-remote-* "$OUTPUT_DIR"/bmuxd-remote-checksums.txt "$OUTPUT_DIR"/bmuxd-remote-manifest.json
...
CHECKSUMS_ASSET_NAME="bmuxd-remote-checksums${SUFFIX_TAG}.txt"
CHECKSUMS_PATH="${OUTPUT_DIR}/${CHECKSUMS_ASSET_NAME}"
MANIFEST_PATH="${OUTPUT_DIR}/bmuxd-remote-manifest${SUFFIX_TAG}.json"
...
: > "$CHECKSUMS_PATH"
ENTRIES_FILE="$(mktemp "${TMPDIR:-/tmp}/bmuxd-remote-entries.XXXXXX")"
trap 'rm -f "$ENTRIES_FILE"' EXIT
: > "$ENTRIES_FILE"

for target in "${TARGETS[@]}"; do
  read -r GOOS GOARCH <<<"$target"
  ASSET_NAME="bmuxd-remote-${GOOS}-${GOARCH}${SUFFIX_TAG}"
  ...
  BUILD_PATH="${OUTPUT_DIR}/bmuxd-remote-${GOOS}-${GOARCH}.build"
  (
    cd "$DAEMON_ROOT"
    GOOS="$GOOS" \
    GOARCH="$GOARCH" \
...
echo "Built bmuxd-remote assets in ${OUTPUT_DIR}"
