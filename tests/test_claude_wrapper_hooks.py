...
""",
        ...
        env["FAKE_HOOK_CMUX_BIN_LOG"] = str(hook_bmux_bin_log)
        env["FAKE_CMUX_LOG"] = str(bmux_log)
...
def test_live_socket_injects_supported_hooks(failures: list[str]) -> None:
    code, real_argv, bmux_log, stderr, claudecode, node_options, runtime_node_options, child_node_options, hook_bmux_bin = run_wrapper(
        socket_state="live",
        argv=["hello"],
    )
    ...
    for hook_name, expected_subcommand in {
        ...
        expect(
            hook_command == f'"${{CMUX_CLAUDE_HOOK_CMUX_BIN:-bmux}}" claude-hook {expected_subcommand}',
...
def test_live_socket_tmpdir_failure_skips_node_options_injection(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="bmux-claude-wrapper-bad-tmp-") as td:
        ...
        code, real_argv, bmux_log, stderr, claudecode, node_options, runtime_node_options, child_node_options, _ = run_wrapper(
            socket_state="live",
            argv=["hello"],
