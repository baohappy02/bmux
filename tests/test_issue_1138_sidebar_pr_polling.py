...
def _gh_stub() -> str:
    return textwrap.dedent(
        ...
        case "$scenario" in
          prompt_helper_idle)
            printf '1138\\tOPEN\\thttps://github.com/manaflow-ai/bmux/pull/1138\\n'
            ;;
          ...
          branch_switch_clear)
            if [ "$branch" = "feature/old" ]; then
              printf '111\\tOPEN\\thttps://github.com/manaflow-ai/bmux/pull/111\\n'
              exit 0
            fi
...
def _shell_command(kind: str, scenario: str) -> str:
    ...
    if kind == "zsh":
        return textwrap.dedent(
            ...
            _bmux_send() {{ print -r -- "$1" >> "$CMUX_TEST_SEND_LOG"; }}
            _bmux_prompt_entry() {{ _bmux_precmd; }}
            _bmux_nested_shell_entry() {{ _bmux_preexec zsh; }}
            _bmux_regular_command_entry() {{ _bmux_preexec "echo hi"; }}
    ...
    if kind == "bash":
        return textwrap.dedent(
            ...
            _bmux_nested_shell_entry() {{ _bmux_bash_preexec_hook bash; }}
            _bmux_regular_command_entry() {{ _bmux_bash_preexec_hook "echo hi"; }}
...
def _report_line(number: int) -> str:
    return (
        f"report_pr {number} https://github.com/manaflow-ai/bmux/pull/{number} "
