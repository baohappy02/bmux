...
python3 - <<'PY' "$OUTPUT_DIR/bmuxd-remote-manifest.json" "$OUTPUT_DIR/bmuxd-remote-checksums.txt"
import json
import sys
...
if not manifest["checksumsURL"].endswith("/bmuxd-remote-checksums.txt"):
...
# ------------------------------------------------------------------
SUFFIX_DIR="$(mktemp -d "${TMPDIR:-/tmp}/bmux-remote-assets-suffix-test.XXXXXX")"
...
for asset in \
  bmuxd-remote-darwin-arm64-123456 \
  bmuxd-remote-darwin-amd64-123456 \
  bmuxd-remote-linux-arm64-123456 \
  bmuxd-remote-linux-amd64-123456 \
  bmuxd-remote-checksums-123456.txt \
  bmuxd-remote-manifest-123456.json
do
...
python3 - <<'PY' "$SUFFIX_DIR/bmuxd-remote-manifest-123456.json"
import json
import sys
from pathlib import Path
