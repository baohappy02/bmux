...
def run_bash(shell_dir: Path, real_bin: Path, log_path: Path) -> tuple[int, str, list[str]]:
    ...
    result = subprocess.run(
        [
            ...
            "--norc",
            "-c",
            f'source "{shell_dir / "bmux-bash-integration.bash"}"; PATH="$CMUX_TEST_REAL_BIN:$PATH"; claude bash-case',
        ],
        env=env,
        capture_output=True,
        text=True,
...
def run_bash_with_alias(shell_dir: Path, real_bin: Path, log_path: Path) -> tuple[int, str, list[str]]:
    ...
    result = subprocess.run(
        [
            ...
            "--norc",
            "-ic",
            f'alias claude="$CMUX_TEST_REAL_BIN/user-claude"; source "{shell_dir / "bmux-bash-integration.bash"}"; '
