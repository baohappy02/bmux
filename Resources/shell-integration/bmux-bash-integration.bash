...
_bmux_ports_kick() {
    ...
    [[ -n "$CMUX_TAB_ID" ]] || return 0
    [[ -n "$CMUX_PANEL_ID" ]] || return 0
    _CMUX_PORTS_LAST_RUN="$(_bmux_now)"
    {
        _bmux_send "ports_kick --tab=$CMUX_TAB_ID --panel=$CMUX_PANEL_ID"
    } >/dev/null 2>&1 & disown
}
...
_bmux_clear_pr_for_panel() {
    ...
    _bmux_send "clear_pr --tab=$CMUX_TAB_ID --panel=$CMUX_PANEL_ID"
}
...
_bmux_pr_cache_prefix() {
    ...
    printf '%s\n' "/tmp/bmux-pr-cache-${CMUX_PANEL_ID}"
}

_bmux_pr_force_signal_path() {
    [[ -n "$CMUX_PANEL_ID" ]] || return 1
    printf '%s\n' "/tmp/bmux-pr-force-${CMUX_PANEL_ID}"
}
...
_bmux_report_pr_for_path() {
    ...
    _bmux_send "report_pr $number $url $status_opt --branch=\"$quoted_branch\" --tab=$CMUX_TAB_ID --panel=$CMUX_PANEL_ID"
}
...
_bmux_prompt_command() {
    ...
    if [[ "$pwd" != "$_CMUX_PWD_LAST_PWD" ]]; then
        ...
        {
            local qpwd="${pwd//\"/\\\"}"
            _bmux_send "report_pwd \"${qpwd}\" --tab=$CMUX_TAB_ID --panel=$CMUX_PANEL_ID"
        } >/dev/null 2>&1 & disown
    fi
    ...
    if [[ "$pwd" != "$_CMUX_GIT_HEAD_LAST_PWD" ]]; then
        ...
        _CMUX_GIT_HEAD_PATH="$(_bmux_git_resolve_head_path 2>/dev/null || true)"
        _CMUX_GIT_HEAD_SIGNATURE=""
    fi
    if [[ -n "$_CMUX_GIT_HEAD_PATH" ]]; then
        local head_signature
        head_signature="$(_bmux_git_head_signature "$_CMUX_GIT_HEAD_PATH" 2>/dev/null || true)"
    ...
    if [[ -z "$_CMUX_GIT_JOB_PID" ]] || ! kill -0 "$_CMUX_GIT_JOB_PID" 2>/dev/null; then
        ...
        _CMUX_GIT_LAST_RUN=$now
        {
            ...
            if [[ -n "$branch" ]]; then
                ...
                _bmux_send "report_git_branch $branch $dirty_opt --tab=$CMUX_TAB_ID --panel=$CMUX_PANEL_ID"
            else
                _bmux_send "clear_git_branch --tab=$CMUX_TAB_ID --panel=$CMUX_PANEL_ID"
            fi
        } >/dev/null 2>&1 &
        _CMUX_GIT_JOB_PID=$!
        disown
    ...
    if (( should_restart_pr_poll )); then
        _CMUX_PR_FORCE=0
        _bmux_start_pr_poll_loop "$pwd" 1
    fi
...
_bmux_install_prompt_command() {
    ...
    if [[ "$decl" == "declare -a"* ]]; then
        ...
        if (( BASH_VERSINFO[0] > 5 || (BASH_VERSINFO[0] == 5 && BASH_VERSINFO[1] >= 3) )); then
            builtin readonly _CMUX_BASH_PS0='${ _bmux_bash_preexec_hook "$BASH_COMMAND"; }'
        else
            builtin readonly _CMUX_BASH_PS0='$(_bmux_bash_preexec_hook "$BASH_COMMAND" >/dev/null)'
        fi
