...
_bmux_ports_kick() {
    ...
    _bmux_send_bg "ports_kick --tab=$CMUX_TAB_ID --panel=$CMUX_PANEL_ID"
}
...
_bmux_clear_pr_for_panel() {
    ...
    [[ -n "$CMUX_TAB_ID" ]] || return 0
    [[ -n "$CMUX_PANEL_ID" ]] || return 0
    _bmux_send_bg "clear_pr --tab=$CMUX_TAB_ID --panel=$CMUX_PANEL_ID"
}
...
_bmux_pr_cache_prefix() {
    ...
    print -r -- "/tmp/bmux-pr-cache-${CMUX_PANEL_ID}"
}

_bmux_pr_force_signal_path() {
    [[ -n "$CMUX_PANEL_ID" ]] || return 1
    print -r -- "/tmp/bmux-pr-force-${CMUX_PANEL_ID}"
}
...
_bmux_start_git_head_watch() {
    ...
    watch_head_path="$(_bmux_git_resolve_head_path 2>/dev/null || true)"
        ...
        while true; do
            ...
            if [[ -n "$signature" && "$signature" != "$last_signature" ]]; then
                ...
                if [[ -n "$_CMUX_PR_POLL_PID" ]] && kill -0 "$_CMUX_PR_POLL_PID" 2>/dev/null; then
                    ...
                    _bmux_run_pr_probe_with_timeout "$watch_pwd" 1 || true
                fi
            fi
        done
    } >/dev/null 2>&1 &!
...
_bmux_precmd() {
    ...
    if [[ "$pwd" != "$_CMUX_PWD_LAST_PWD" ]]; then
        ...
        local qpwd="${pwd//\"/\\\"}"
        _bmux_send_bg "report_pwd \"${qpwd}\" --tab=$CMUX_TAB_ID --panel=$CMUX_PANEL_ID"
    fi
