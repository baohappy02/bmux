...
<p align="center"><strong>bmux</strong> is an independent fork of cmux for private development. This repo currently tracks a local rebrandable snapshot and keeps the original GPL-3.0-or-later license.</p>
...
Use `/Applications/bmux.app` as the local runtime target.

Rebuild, install, and relaunch the app:

```bash
./scripts/reloadp.sh
```

Clean leftover local build artifacts when needed:
