...
SOCKET_PATH = os.environ.get("CMUX_SOCKET", "/tmp/bmux-debug.sock")
...
def _find_cli_binary() -> str:
    ...
    if not candidates:
        raise bmuxError("Could not locate bmux CLI binary; set CMUXTERM_CLI")
    ...
    with bmux(SOCKET_PATH) as client:
        try:
            ...
            _must(
                ...
                f"bmux ssh should select the newly created workspace: expected {workspace_id}, got {selected_workspace_id}",
            )
            ...
            _must(
                ...
                f"bmux ssh should launch startup command via interactive zsh for shell integration: {ssh_startup_command!r}",
            )
            ...
            _must(
                ...
                f"bmux ssh should route RemoteCommand through /bin/sh for non-POSIX login shells: {ssh_command!r}",
            )
            _must(
                f"export PATH=\"$HOME/.bmux/bin:$PATH\"" in ssh_command,
                f"bmux ssh should still prepend the remote bmux wrapper path: {ssh_command!r}",
            )
            _must(
                f"export CMUX_SOCKET_PATH=127.0.0.1:{int(remote_relay_port)}" in ssh_command,
                f"bmux ssh should still pin the relay socket path in RemoteCommand: {ssh_command!r}",
            )
            _must(
                "case \"${CMUX_LOGIN_SHELL##*/}\" in" in ssh_command,
                f"bmux ssh should still branch on the user's login shell when possible: {ssh_command!r}",
            )
            _must(
                "cat > \"$bmux_shell_dir/.zshrc\"" in ssh_command,
                f"bmux ssh should install a post-rc zsh wrapper so the remote bmux wrapper stays first on PATH: {ssh_command!r}",
            )
            _must(
                "bmux_wait_attempt=0" in ssh_command,
                f"bmux ssh should wait briefly for the authenticated relay before showing the remote shell: {ssh_command!r}",
            )
            _must(
                "exec \"$CMUX_LOGIN_SHELL\" --rcfile \"$bmux_shell_dir/.bashrc\" -i" in ssh_command,
                f"bmux ssh should still support bash login shells with a post-rc wrapper file: {ssh_command!r}",
            )
            _must(
                "exec \"$CMUX_LOGIN_SHELL\" -i" in ssh_command,
                f"bmux ssh should still hand off to the user's interactive login shell when possible: {ssh_command!r}",
            )
            ...
            while time.time() < deadline_daemon:
                ...
                raise bmuxError(f"unreachable host should drive daemon state to error: {last_status}")
