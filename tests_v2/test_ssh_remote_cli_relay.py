...
def _assert_remote_ping(host: str, host_port: int, key_path: Path, remote_socket_addr: str, *, label: str) -> None:
    ping_result = _ssh_run(
        ...
        f"CMUX_SOCKET_PATH={remote_socket_addr} $HOME/.bmux/bin/bmux ping",
        check=False,
    )
    _must(
    ...
    try:
        ...
        with bmux(SOCKET_PATH) as client:
            ...
            list_ws_result = _ssh_run(
                host, host_ssh_port, key_path,
                f"CMUX_SOCKET_PATH={remote_socket_addr} $HOME/.bmux/bin/bmux --json list-workspaces",
                check=False,
            )
            _must(
            ...
            new_win_result = _ssh_run(
                host, host_ssh_port, key_path,
                f"CMUX_SOCKET_PATH={remote_socket_addr} $HOME/.bmux/bin/bmux new-window",
                check=False,
            )
            _must(
            ...
            rpc_result = _ssh_run(
                host, host_ssh_port, key_path,
                f"CMUX_SOCKET_PATH={remote_socket_addr} $HOME/.bmux/bin/bmux rpc system.capabilities",
                check=False,
            )
            _must(
